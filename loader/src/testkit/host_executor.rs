...
impl HostExecutor<'_> {
    ...
    fn tile_map(&mut self, instr: &StorageInstr) -> Result<(), Error> {
        ...
        let output = match kind {
            ...
            // Encode is the one transform with two outputs — the payload and
            // the scales it cannot be read without — so it writes them itself
            // rather than flowing into the single-output plumbing below.
            TileMapKind::Encode => {
