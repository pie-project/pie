...
impl pie::inferlet::working_set::HostKvWorkingSet for InstanceState {
    ...
    async fn discard(
        ...
        if out.is_ok() {
            if let Some(orchestrator) = crate::inference::contention::contention() {
    ...
    async fn slice(
        ...
        let sliced = stores
            ...
            .lock()
            .unwrap()
            .slice(ws.id, range.start as u64..(range.start as u64 + range.len as u64));
        match sliced {
            Ok(id) => {
                let child = KvWorkingSet { id, ..ws };
                Ok(Ok(self.ctx().table.push(child)?))
    ...
    async fn copy_into(
        ...
        crate::ptir::ptir_host::working_set_copy_into(
            self,
            this,
            on,
            dst_page_ids,
