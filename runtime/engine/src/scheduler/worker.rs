...
impl InFlightControls {
    ...
    /// Whether `item` may be posted into this set now.
    fn admits(&self, item: &QueuedItem) -> bool {
        if BatchScheduler::standalone_copy(item) {
            self.admits_copy()
        } else {
            self.is_empty()
        }
        ...
        loop {
            ...
            match item {
                QueuedItem::Launch(_) => {
                    ...
                    // it.
                    if Self::rotate_launch_for_wave_work(pending, in_flight_control.is_empty()) {
                        progress = true;
                        continue;
                    }
                    break;
                ...
                QueuedItem::CloseInstance { id, .. } => {
                    ...
                    if in_flight_control.is_settling() {
                        break;
                    }
                    let id = *id;
                    if hold_closes {
                ...
                // pool resize) blocks the next control, and nothing joins a
                // standalone copy except another standalone copy. Only
                // async-completing controls are ever in flight here —
                // lifecycle controls execute on the lane without entering
                // the set, their driver order guaranteed by the lane FIFO.
