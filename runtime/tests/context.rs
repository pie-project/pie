...
async fn registered_test_pid() -> uuid::Uuid {
    let pid = test_pid();
...
fn create_and_save_and_open() {
    let s = state();
    s.rt.block_on(async {
        let id = pie::context::create(MODEL, registered_test_pid().await)
            .await
            .unwrap();
        ...
        let snapshot_id = pie::context::lookup(MODEL, USER.to_string(), "test-ctx".into())
            .await
            .unwrap();
        let found = pie::context::fork(MODEL, snapshot_id, registered_test_pid().await).await;
        assert!(found.is_ok());
    });
}
...
fn destroy_removes_context() {
    ...
    s.rt.block_on(async {
        ...
        let fork_result = pie::context::fork(MODEL, id, registered_test_pid().await).await;
        assert!(
            fork_result.is_err(),
            "fork from destroyed context should fail"
        );
...
fn fork_context() {
    let s = state();
    s.rt.block_on(async {
        let parent_id = pie::context::create(MODEL, registered_test_pid().await)
            .await
            .unwrap();

        let child_id = pie::context::fork(MODEL, parent_id, registered_test_pid().await)
            .await
            .unwrap();
...
fn full_page_lifecycle() {
    ...
    s.rt.block_on(async {
        ...
        let pid = registered_test_pid().await;
        ...
        let child_id = pie::context::fork(MODEL, id, registered_test_pid().await)
            .await
            .unwrap();
