...
fn gate_overlapping_read_write() {
    ...
    let mut txn = a.txn_begin();
    let (out_obj, _mp) = ws.cow_write_slot(1, &mut txn, &mut a).unwrap();
...
fn gate_cow_before_write() {
    ...
    let mut txn = a.txn_begin();
    let (new_obj, mp) = ws1.cow_write_slot(0, &mut txn, &mut a).unwrap();
...
fn gate_driver_failure_abort() {
    ...
    ws1.resolve_write(&[0], cur_gen).unwrap();
    let mut txn = a.txn_begin();
    let (_new_obj, mp) = ws1.cow_write_slot(0, &mut txn, &mut a).unwrap();
    assert!(mp.is_some());
...
fn gate_prepare_validation_edges() {
    ...
    // A structural mutation bumps the generation → a write captured against the
    // old generation is rejected (stale-mutation guard).
    ws.alloc(1).unwrap();
    assert!(
