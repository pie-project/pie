...
//!
//! GOLDEN CONTRACT — DO NOT DELETE WITHOUT PORTING (plan W2.2 / W4). The tests
//! at the bottom (`golden_charlie_fork_freeze_csrs`, `golden_continue_tail`,
//! `golden_page_turn`, `golden_fork_freeze`, …) are the LOCKED wire-form
//! geometry contract. Their driver twin (`driver/cuda/tests/beam_csrs_test.cpp`)
//! was deleted with the beam island (W1.5), so THIS FILE is now the sole home of
//! these vectors. They must be re-expressed as SDK trace tests (assert the
//! in-graph program's emitted wire-form port values, W2.1/W2.2) BEFORE the beam
//! host-replay path is deleted (W4). Until then this module doubles as the CPU
//! reference the in-graph program must reproduce.
//!
