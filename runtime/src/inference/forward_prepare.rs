...
mod tests {
    ...
    fn w(slot: u32, page: u32, valid: u32) -> KvWrite {
        KvWrite { slot_index: slot, page, valid_len: valid }
    }

    #[test]
    fn prefill_single_full_page() {
        let p = project_kv(&[], 0, &[w(0, 100, 16)], 16).unwrap();
        assert_eq!(p.physical_page_ids, vec![100]);
        assert_eq!(p.last_page_len, 16);
        assert_eq!(p.active_page_idx, Some(0));
        assert_eq!(p.full_page_writes, vec![0]);
    }

    #[test]
    fn prefill_partial_page_not_sealable() {
        let p = project_kv(&[], 0, &[w(0, 100, 5)], 16).unwrap();
        assert_eq!(p.last_page_len, 5);
        assert!(p.full_page_writes.is_empty());
    }

    #[test]
    fn decode_append_after_context() {
        let p = project_kv(&[100, 101], 32, &[w(2, 102, 1)], 16).unwrap();
        assert_eq!(p.physical_page_ids, vec![100, 101, 102]);
        assert_eq!(p.last_page_len, 1);
        assert_eq!(p.active_page_idx, Some(2));
        assert!(p.full_page_writes.is_empty());
    }

    #[test]
    fn multi_page_append() {
        let p = project_kv(&[100], 16, &[w(1, 101, 16), w(2, 102, 4)], 16).unwrap();
        assert_eq!(p.physical_page_ids, vec![100, 101, 102]);
        assert_eq!(p.last_page_len, 4);
        assert_eq!(p.active_page_idx, Some(2));
        assert_eq!(p.full_page_writes, vec![1]);
    }

    #[test]
    fn overlap_rewrite_within_context() {
        let p = project_kv(&[100, 101, 102], 48, &[w(1, 200, 16)], 16).unwrap();
        assert_eq!(p.physical_page_ids, vec![100, 200, 102]);
        assert_eq!(p.last_page_len, 16);
        assert_eq!(p.active_page_idx, Some(1));
        assert_eq!(p.full_page_writes, vec![1]);
    }

    #[test]
    fn read_only_forward_has_no_active_page_idx() {
        let p = project_kv(&[100, 101], 20, &[], 16).unwrap();
        assert_eq!(p.physical_page_ids, vec![100, 101]);
        assert_eq!(p.last_page_len, 4);
        assert_eq!(p.active_page_idx, None);
        assert!(p.full_page_writes.is_empty());
    }

    #[test]
    fn non_contiguous_output_is_rejected() {
        let err = project_kv(&[100], 16, &[w(3, 103, 1)], 16).unwrap_err();
        assert_eq!(err, PrepareError::NonContiguousActiveRun { gap_at: 1 });
    }

    #[test]
    fn duplicate_output_index_is_rejected() {
        let err = project_kv(&[], 0, &[w(0, 100, 16), w(0, 101, 4)], 16).unwrap_err();
        assert_eq!(err, PrepareError::DuplicateOutputIndex(0));
    }

    #[test]
    fn invalid_valid_len_is_rejected() {
        let zero = project_kv(&[], 0, &[w(0, 100, 0)], 16).unwrap_err();
        assert_eq!(zero, PrepareError::InvalidValidLen { index: 0, valid_len: 0, page_size: 16 });
        let over = project_kv(&[], 0, &[w(0, 100, 17)], 16).unwrap_err();
        assert_eq!(over, PrepareError::InvalidValidLen { index: 0, valid_len: 17, page_size: 16 });
    }

    #[test]
    fn empty_forward_is_rejected() {
        assert_eq!(project_kv(&[], 0, &[], 16).unwrap_err(), PrepareError::EmptyForward);
    }

    #[test]
    fn stale_generation_check() {
        assert!(check_generation(5, 5).is_ok());
        assert_eq!(
            check_generation(5, 7).unwrap_err(),
            PrepareError::StaleGeneration { captured: 5, current: 7 }
        );
    }

    #[test]
    fn invariant_total_kv_matches() {
        // total_kv = (num_pages - 1) * page_size + last_page_len must equal
        // context_valid_tokens + appended tokens for the standard append case.
        let p = project_kv(&[100, 101], 32, &[w(2, 102, 7)], 16).unwrap();
        let num_pages = p.physical_page_ids.len() as u32;
        let total_kv = (num_pages - 1) * 16 + p.last_page_len;
        assert_eq!(total_kv, 32 + 7);
    }
}
