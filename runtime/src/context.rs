...
/// What opening a snapshot actually cost.
///
/// Eviction *suspends* a context rather than deleting it, and leaves the name
/// resolvable. `open` therefore succeeds on an evicted snapshot and silently
/// replays the missing prefix through real forward passes — indistinguishable,
/// from the caller's side, from a genuine cache hit.
///
/// The runtime already computes everything needed to tell those apart; it just
/// had nowhere to return it. This is that value.
///
/// Counted in pages, not as a boolean, because "was it resident" is not a
/// yes/no question:
///
/// * residency is per page-hash against a content-addressed trie, so a
///   suspended context can fork with nothing replayed when its pages are still
///   held by another live context;
/// * recurrent-state (linear-attention) replay is tracked on an independent
///   path from ordinary KV-page replay, so a model can replay its whole
///   recurrent state while `replayed_pages` is zero.
#[derive(Debug, Clone, Copy, Default, PartialEq, Eq)]
pub struct OpenReport {
    /// Committed pages that were already GPU-resident and were reused.
    pub resident_prefix_pages: u32,
    /// Committed pages that had to be regenerated by replay forward passes.
    pub replayed_pages: u32,
    /// Whether recurrent state was replayed. Independent of `replayed_pages`.
...
pub async fn take(
    ...
    take_with_report(model_idx, username, name, owner)
        .await
        .map(|(id, _)| id)
}

/// `take`, additionally reporting what it cost — see [`OpenReport`].
pub async fn take_with_report(
    model_idx: usize,
    username: String,
    name: String,
    owner: ProcessId,
...
pub async fn fork(model_idx: usize, id: ContextId, owner: ProcessId) -> Result<ContextId> {
    fork_with_report(model_idx, id, owner)
        .await
        .map(|(id, _)| id)
}

/// `fork`, additionally reporting what the fork cost — see [`OpenReport`].
///
/// The runtime computes this either way; `fork` simply drops it so existing
/// callers keep their signature.
pub async fn fork_with_report(
    model_idx: usize,
    id: ContextId,
    owner: ProcessId,
) -> Result<(ContextId, OpenReport)> {
