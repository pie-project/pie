...
use lru::LruCache;
use pie_sampling_ir::ptir::container::{self, ContainerDecodeError, TraceContainer};
use pie_sampling_ir::ptir::container_hash;
use pie_sampling_ir::ptir::registry::ModelProfile;
use pie_sampling_ir::ptir::validate::{bind, BoundTrace, ValidateError};
...
impl PtirRegistry {
    ...
    pub fn register(
        ...
        let sidecar = pie_sampling_ir::ptir::sidecar::encode_bound(&bound);
...
fn price(c: &TraceContainer) -> Pricing {
    ...
    let rows = c
        .ports
        .iter()
        .find(|p| p.port == pie_sampling_ir::ptir::registry::Port::EmbedIndptr)
...
mod tests {
    ...
    fn sidecar_roundtrips_and_matches_identity() {
        ...
        let decoded = pie_sampling_ir::ptir::sidecar::decode_bound(&prog.sidecar).unwrap();
