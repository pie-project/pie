...
use pie_sampling_ir::ptir::container::{self, HostRole};
...
impl PtirInstance {
    ...
    /// program from cache and drives the already-built, persistent arena.
    /// `host_puts` are the D1-coalesced host `channel.put`s staged for this fire.
    pub fn submission(
        &self,
        first_fire: bool,
...
mod tests {
    use super::*;
    use crate::api::ptir_registry::{register, PtirRegistry};
    ...
    use pie_sampling_ir::ptir::registry::{ModelProfile, Port, Stage};
    ...
    fn fire_geometry_prefills_request_from_seed() {
        ...
        req.push_ptir_program(&inst.submission(true, vec![]));
    ...
    fn submission_carrier_first_fire_vs_steady_state() {
        ...
        let first = inst.submission(true, vec![]);
        ...
        let steady = inst.submission(false, vec![]);
    ...
    fn register_instantiate_run_on_mock_interp() {
        ...
        use pie_sampling_ir::ptir::interp::{Instance as Interp, NoKernels, PassInputs};
