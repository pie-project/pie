...
use pie_sampling_ir::ptir::container::{PortSource, TraceContainer};
...
pub fn map_geometry(
    ...
    // -- token family --
    let tokens = resolve(container, values, Port::EmbedTokens)?
        .ok_or(GeometryError::NoEmbed)?;
    ...
    if let Some(b) = resolve(container, values, Port::Pages)? {
        g.kv_page_indices = as_u32(Port::Pages, &b)?;
    }
    if let Some(b) = resolve(container, values, Port::PageIndptr)? {
        g.kv_page_indptr = as_u32(Port::PageIndptr, &b)?;
    }
    if let Some(b) = resolve(container, values, Port::KvLen)? {
