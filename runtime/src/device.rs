...
///
/// Two parallel `pie serve` processes on the same host would otherwise
/// collide on `/pie_shmem_g0` (POSIX shmem is a host-global namespace).
/// `$PIE_SHMEM_NAME` lets a launcher (e.g. test harness) namespace each
/// run; the default keeps the legacy `/pie_shmem` base.
