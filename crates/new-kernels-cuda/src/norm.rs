...
const FILE: &str = "norm/rmsnorm.cuh";

const ELEMENTWISE_FILE: &str = "norm/elementwise.cuh";
...
/// or offset by one (`w + 1`, Gemma-style).
const ABSOLUTE_BANK: &str = "rmsnorm";

const OFFSET_BANK: &str = "rmsnorm_gemma";
...
fn rms_row(
    ...
    ctx.fire(
        op,
        Fire::at(FILE, symbol(&format!("::pie::norm::{template}<{t}, 256>"))).apply(launch),
...
pub fn rmsnorm(
    ...
    rms_row(ctx, "norm.rmsnorm", ABSOLUTE_BANK, x, weight, y, 0, eps)
}
...
pub fn rmsnorm_no_scale(
    ...
    ctx.fire(
        ...
        Fire::at(
            FILE,
            symbol(&format!("::pie::norm::rmsnorm_no_scale<{t}, 256>")),
        )
        .apply(launch),
...
pub fn rmsnorm_gated(
    ...
    ctx.fire(
        ...
        Fire::at(
            FILE,
            symbol(&format!("::pie::norm::rmsnorm_gated_f32_in<{t}, 256>")),
        )
        .apply(launch),
        &[
            x.arg(),
...
pub fn rmsnorm_gated_by(
    ...
    ctx.fire(
        ...
        Fire::at(
            "ssm/kda.cuh",
            symbol(&format!("::pie::ssm::kda_o_norm_gated<{t}>")),
        )
        .apply(Launch::grid(
            [y.rows, heads, 1],
...
pub fn residual_add(ctx: &Ctx, x: Tensor, y: &mut Tensor) -> Result<(), KernelError> {
    ...
    ctx.fire(
        ...
        Fire::at(
            ELEMENTWISE_FILE,
            symbol(&format!("::pie::norm::residual_add<{t}>")),
        )
        .apply(launch),
        &[y.arg(), x.arg(), n.arg()],
    )
...
pub fn add_bias(ctx: &Ctx, bias: Tensor, out: &mut Tensor) -> Result<(), KernelError> {
    ...
    ctx.fire(
        ...
        Fire::at(
            "norm/add_bias.cuh",
            symbol(&format!("::pie::norm::add_bias<{t}>")),
        )
...
pub fn mul_scalar(ctx: &Ctx, s: f32, x: &mut Tensor) -> Result<(), KernelError> {
    ...
    ctx.fire(
        ...
        Fire::at(
            ELEMENTWISE_FILE,
            symbol(&format!("::pie::norm::scalar_mul<{t}>")),
        )
        .apply(launch),
        &[x.arg(), s.arg(), n.arg()],
    )
...
pub fn scale(ctx: &Ctx, s: Tensor, x: &mut Tensor) -> Result<(), KernelError> {
    ...
    ctx.fire(
        ...
        Fire::at(
            ELEMENTWISE_FILE,
            symbol(&format!("::pie::norm::scale<{t}>")),
        )
        .apply(launch),
        &[x.arg(), s.arg(), n.arg()],
    )
...
/// this entry is the seam, [`crate::attn::res_blend`] is the launch.
pub fn res_blend(
    ctx: &Ctx,
    prefix: Tensor,
    blocks: &[Tensor],
