...
//! entry per IR variant; [`RecurrentPool`] is the state, updated in place.
...
const CONV_FILE: &str = "ssm/causal_conv1d.cuh";

const PREP_FILE: &str = "ssm/gated_delta_net_prep.cuh";

const GDN_FILE: &str = "ssm/gated_delta_net.cuh";

const KDA_FILE: &str = "ssm/kda.cuh";
...
pub fn causal_conv1d(
    ...
    ctx.fire(
        ...
        .apply(Launch::grid(
            [channels.div_ceil(BLOCK), rows, 1],
            [BLOCK, 1, 1],
        )),
        &[
            x.arg(),
...
pub fn causal_conv1d_chunked(
    ...
    ctx.fire(
        OP,
        Fire::at(CONV_FILE, entrypoint).apply(launch),
        &[
            x.data.arg(),
            weight.arg(),
...
pub fn gdn_prep(
    ...
    ctx.fire(
        OP,
        Fire::at(PREP_FILE, "::pie::ssm::qwen_gdn_ba_gates<::pie::bf16>").apply(Launch::grid(
...
impl Delta {
    ...
    fn stage(
        ...
        let qk = plane(ctx, op, "ssm::gdn_chunk_qk", 2 * key)?;
        let v = plane(ctx, op, "ssm::gdn_chunk_v", val)?;
        let gb = plane(ctx, op, "ssm::gdn_chunk_gates", 2 * decay)?;
        ...
        ctx.fire(
            op,
            Fire::at(PREP_FILE, "::pie::ssm::qwen_gdn_qk_norm<::pie::bf16, 128>")
        ...
        ctx.fire(
            op,
            Fire::at(PREP_FILE, "::pie::ssm::qwen_gdn_v_gates<::pie::bf16, 128>")
...
/// this plane gates afterwards (`norm.rmsnorm_gated`), so it goes unread —
/// as before.
...
pub fn gated_delta(
    ...
    ctx.fire(
        OP,
        Fire::at(GDN_FILE, entrypoint).apply(launch),
        &[
            ArgValue::Ptr(staged.q_norm),
            ArgValue::Ptr(staged.k_norm),
            ArgValue::Ptr(staged.v),
...
pub fn gated_delta_chunked(
    ...
    let (q_norm, k_norm) = if v_heads == k_heads {
        ...
        let repeated = plane(ctx, OP, "ssm::gdn_chunk_repeat", 2 * wide)?;
...
impl Kda {
    ...
    fn stage(
        ...
        let qkv = plane(ctx, op, "ssm::kda_qkv", 3 * wide)?;
        let gb = plane(ctx, op, "ssm::kda_gates", wide + decay)?;
        ...
        ctx.fire(
            op,
            Fire::at(KDA_FILE, "::pie::ssm::kda_qkv_prep<::pie::bf16, 128>")
        ...
        ctx.fire(
            op,
            Fire::at(KDA_FILE, "::pie::ssm::kda_gate_beta<::pie::bf16>").apply(Launch::grid(
                [self.n, self.heads, 1],
...
pub fn kda_step(
    ...
    ctx.fire(
        OP,
        Fire::at(KDA_FILE, "::pie::ssm::kda_recurrent_step_batched").apply(
...
pub fn kda_chunked(
    ...
    ctx.fire(
        OP,
        Fire::at(KDA_FILE, "::pie::ssm::kda_prefill_batched").apply(
            Launch::grid(
                [lanes, shape.heads, 1],
