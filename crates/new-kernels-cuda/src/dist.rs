//! `Dist`: the tensor-parallel collectives, NCCL on the stream — the "enable
//! nccl" item, landed (design §9). One entry per IR variant; every rank
//! traces the same plan, so a collective here is a sync point of an SPMD
//! fire, enqueued like any launch and never synchronised on.
//!
...
pub fn reduce_scatter(ctx: &Ctx, x: Tensor, y: &mut Tensor) -> Result<(), KernelError> {
    const OP: &str = "dist.reduce_scatter";
    let comm = ctx.comm(OP)?;
...
/// whole fire under graph capture (the `gemm` precedent).
#[cfg(feature = "_cuda")]
