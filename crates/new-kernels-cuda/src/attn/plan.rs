...
//!
//! - host copies of the geometry the IR names (`kv_indptr`, `qo_indptr`) —
//!   the planners walk their contents, and device handles cannot be read
//!   host-side (see the MENLO-SEAM notes on each builder);
...
/// The custom-mask pair `attention.masked` reads. The IR's `Masked` variant
/// names no mask operand, so the driver binds its fire mask onto the
/// prefill plan at build time — the same seam metal's fire tables cross.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub struct Mask {
    /// Packed `u8` mask bits.
    pub bits: Tensor,

    /// `i32`, `[lanes + 1]`: each request's span of `bits`.
    pub indptr: Tensor,
}
...
pub struct PrefillPlan {
    ...
    /// Bound only when this plan serves `attention.masked`.
...
impl MlaPlan {
    pub fn stage(&self, ctx: &Ctx) -> Result<(), KernelError> {
        upload(ctx, "mla.plan", &self.int_upload, self.workspace.int_ptr)
    }
}
...
pub(crate) fn refuse_len(
    op: &'static str,
    which: &'static str,
    len: usize,
    batch_size: u32,
) -> Result<(), KernelError> {
    let needed = batch_size as usize + 1;
    if len < needed {
        return Err(refuse(
            op,
            format!("the host {which} holds {len} entries for a batch of {batch_size}"),
        ));
    }
    Ok(())
}
...
/// cannot live inside a pure builder).
///
// MENLO-SEAM: `attention.plan_decode` in the IR names on-device geometry
// (kv_indptr/kv_indices/last_page_len); this builder walks `kv_indptr`'s
// CONTENTS, so the driver binds a host copy beside the device tensor.
// kv_indices and last_page_len are never read at build time — they ride the
// pool row into the launch.
...
pub fn plan_decode(
    ...
    let built = if !windowed_split
        && can_use_static_nonsplit(shape.num_requests, device.cc_major, enable_cuda_graph)
    {
        static_nonsplit_decode(OP, &shape, enable_cuda_graph, workspace.int_bytes)?
    } else {
...
/// The old static-nonsplit fast path: small batches on cc >= 8 skip the
/// planner entirely — one work item per request, no kv split.
#[must_use]
fn can_use_static_nonsplit(num_requests: u32, cc_major: u32, enable_cuda_graph: bool) -> bool {
    !enable_cuda_graph && !force_split_kv_small_enabled() && cc_major >= 8 && num_requests <= 512
}

fn static_nonsplit_decode(
    op: &'static str,
    shape: &Shape,
    enable_cuda_graph: bool,
    int_bytes: usize,
) -> Result<Built<DecodePlanInfo>, KernelError> {
    let n = shape.num_requests as usize;

    let mut int_alloc = alloc::AlignedAllocator::new(op, int_bytes);
    let request_indices_offset = int_alloc.alloc(4 * n, 16, "batch_decode_request_indices")? as i64;
    let kv_tile_indices_offset = int_alloc.alloc(4 * n, 16, "batch_decode_kv_tile_indices")? as i64;
    let o_indptr_offset = int_alloc.alloc(4 * (n + 1), 16, "batch_decode_o_indptr")? as i64;
    let kv_chunk_size_ptr_offset = int_alloc.alloc(4, 1, "batch_decode_kv_chunk_size_ptr")? as i64;
    let int_bytes = int_alloc.used();

    let mut staging = alloc::Staging::new(op, int_bytes);
    let request_indices: Vec<i32> = (0..n as i32).collect();
    let kv_tile_indices = vec![0i32; n];
    let o_indptr: Vec<i32> = (0..=n as i32).collect();
    staging.put_i32s(
        request_indices_offset as usize,
        &request_indices,
        "batch_decode_request_indices",
    )?;
    staging.put_i32s(
        kv_tile_indices_offset as usize,
        &kv_tile_indices,
        "batch_decode_kv_tile_indices",
    )?;
    staging.put_i32s(o_indptr_offset as usize, &o_indptr, "batch_decode_o_indptr")?;
    staging.put_i32(
        kv_chunk_size_ptr_offset as usize,
    ...
    Ok(Built {
        info: DecodePlanInfo {
            enable_cuda_graph,
            split_kv: false,
            padded_batch_size: i64::from(shape.num_requests),
...
// copies of both indptrs; the driver binds them beside the device tensors
// the IR names.
#[allow(clippy::too_many_arguments)]
pub fn plan_prefill(
    qo_indptr: &[i32],
    ...
    let (built, capturable) =
        match prefill::plan(OP, &req, device, workspace.int_bytes, workspace.float_bytes) {
            ...
            Err(_) if enable_cuda_graph => {
                ...
                let built =
                    prefill::plan(OP, &req, device, workspace.int_bytes, workspace.float_bytes)?;
                (built, false)
            }
...
/// struct kind names.
///
// MENLO-SEAM: `kv_len` (per-request kv lengths in tokens) has no IR seat;
// the driver derives it host-side from kv_indptr + last_page_len, as the
// old plane did, and hands the slice in.
#[allow(clippy::too_many_arguments)]
pub fn plan_prefill_sm90(
    ...
    let built = sm90::plan(OP, &req, device, workspace.int_bytes)?;
    Ok(PrefillPlanSm90 {
        info: built.info,
        int_upload: built.int_upload,
        int_bytes: built.int_bytes,
...
/// one token per lane) and prefill.
///
// MENLO-SEAM: as `plan_prefill_sm90` — host slices for qo_indptr, the kv
// element offsets, and the derived per-request kv lengths.
#[allow(clippy::too_many_arguments)]
pub fn plan_mla(
    qo_indptr: &[i32],
    kv_indptr: &[i32],
    ...
    let built = mla::plan(OP, &req, device, workspace.int_bytes, workspace.float_bytes)?;
    Ok(MlaPlan {
        info: built.info,
        int_upload: built.int_upload,
        int_bytes: built.int_bytes,
