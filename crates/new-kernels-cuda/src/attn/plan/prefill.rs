//! The fa2 prefill work split: FlashInfer's host planner, transcribed. Tiles
//! the packed query axis, binary-searches the kv chunk size that fills the
//! grid, and stages the tile/merge index vectors the prefill kernel and the
//! cascade merge walk.
...
use super::{Built, Device, Sizes, refuse_len};
...
impl Request<'_> {
    fn check(&self, op: &'static str) -> Result<(), KernelError> {
        refuse_len(op, "qo_indptr", self.qo_indptr.len(), self.batch_size)?;
...
pub fn split_qo_kv_indptr(
    ...
    cc_major: u32,
) -> Result<Split, KernelError> {
    #[must_use]
    fn binary_search_kv_chunk_size(
    ...
    for i in 0..batch_size {
        ...
        if kv_len_arr[i] < 0 {
            return Err(refuse(op, "the host kv_indptr is not monotone"));
        }
    }
...
pub fn workspace_size(
    ...
    let built = plan_impl(op, req, device, usize::MAX, usize::MAX, Staging::sizing(op))?;
    Ok(Sizes {
...
fn plan_impl(
    ...
    let split = split_qo_kv_indptr(op, req, max_batch_size_if_split, device.cc_major)?;
