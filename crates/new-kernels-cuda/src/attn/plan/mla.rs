//! The latent-attention scheduler: FlashInfer's MLA planner, transcribed.
//! Splits packed query tiles over CTA clusters by a cost heap, carves kv
//! spans at `kv_len_limit`, and books the partial-output merge that the
//! split spans need.
...
use super::{Built, Device, refuse_len};
...
pub fn schedule(
    ...
    refuse_len(op, "qo_indptr", req.qo_indptr.len(), req.batch_size)?;
    refuse_len(op, "kv_indptr", req.kv_indptr.len(), req.batch_size)?;
    if req.kv_len_arr.len() < batch_size {
        return Err(refuse(
            op,
            format!(
                "the host kv length table holds {} entries for a batch of {batch_size}",
                req.kv_len_arr.len()
            ),
        ));
    }
    ...
    for i in 0..batch_size {
        ...
        if qo_len < 0 {
            return Err(refuse(op, "the host qo_indptr is not monotone"));
        }
    ...
    if i64::from(merge_cta_counter) > i64::from(device.num_sm) {
        return Err(refuse(
            op,
            format!(
                "the schedule booked {merge_cta_counter} merge CTAs on a device with {}",
                device.num_sm
            ),
        ));
    }
...
pub fn plan(
    ...
    let writes: [(i64, &Vec<i32>, &'static str); 14] = [
    ...
    let mut float_alloc = AlignedAllocator::new(op, float_bytes);
