//! A transcription of libstdc++'s heap, kept bug-for-bug: the schedules'
//! tie-breaks depend on the exact pop order `std::pop_heap` produces, and a
//! tidier Rust heap would balance the same costs onto different CTAs than
//! the reference planner did.
...
/// The load balancer's `(cta, accumulated cost)` min-heap.
#[derive(Clone, Debug)]
pub struct MinHeap {
    heap: Vec<(i32, f32)>,
}
