...
//! heap, one list per SM. Transcribed so `Struct(AttnPrefillPlanSm90)` has
//! an honest payload; the launcher this schedule feeds was never part of
//! the lattice (the entry answers a typed refusal — see `attn::prefill_sm90`).
...
pub fn schedule(
    ...
    for i in 0..batch_size {
        ...
        if kv_len < 0 {
            return Err(refuse(op, "a host kv length is negative"));
        }
        if qo_len < 0 {
...
pub fn plan(
    ...
    let writes: [(i64, &Vec<i32>, &'static str); 8] = [
        (
            info.qo_tile_indices_offset,
