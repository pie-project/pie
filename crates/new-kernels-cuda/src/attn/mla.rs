...
fn split_kv_a_norm(
    ...
    let kv_lora = stated(
        op,
        nonzero(op, "the latent width this cut states", kv_c.width)?,
    )?;
    ...
    ctx.fire(
        ...
        Fire::at(
            "attn/kimi_mla.cuh",
            crate::jit::symbol(&format!("::pie::attn::split_kv_a_norm<{t}, 256>")),
        )
        .apply(rms(kv_a.rows)),
        &[
            kv_a.arg(),
...
pub fn latents_rope(
    ...
    split_kv_a_norm(ctx, OP, kv_a, weight, eps, kv_c, k_pe)?;
    crate::rope::partial_q(ctx, k_pe, positions, rope_dim, rope_dim, theta)
}
...
pub fn split_q_b(
    ...
    const OP: &str = "mla.split_q_b";
    let t = dtype_dispatch!(OP, q_b.dtype, { Bf16 => "::pie::bf16", F16 => "::pie::f16" });
    let heads = stated(OP, nonzero(OP, "the head count this cut states", heads)?)?;
    ...
    ctx.fire(
        ...
        Fire::at(
            "attn/kimi_mla.cuh",
            crate::jit::symbol(&format!("::pie::attn::split_q_b<{t}>")),
        )
...
pub fn absorb_q(
    ...
    const OP: &str = "mla.absorb_q";
    dtype_dispatch!(OP, q_nope.dtype, { Bf16 => () });
    let heads = stated(OP, nonzero(OP, "the head count this absorb states", heads)?)?;
    let nope = stated(OP, nope_dim)?;
    let v_dim = stated(OP, v_head_dim)?;
    let rank = stated(OP, kv_lora_rank)?;
    let tokens = stated(OP, nonzero(OP, "rows", q_nope.rows)?)?;
...
pub fn absorb_out(
    ...
    const OP: &str = "mla.absorb_out";
    dtype_dispatch!(OP, latent.dtype, { Bf16 => () });
    let heads = stated(OP, nonzero(OP, "the head count this absorb states", heads)?)?;
    let nope = stated(OP, nope_dim)?;
    let v_dim = stated(OP, v_head_dim)?;
    let rank = stated(OP, kv_lora_rank)?;
    let tokens = stated(OP, nonzero(OP, "rows", latent.rows)?)?;
    let handle = ctx.cublas(OP)?;
...
// unread; the appender addresses by the pool's page tables and the fire
// indptr riding in `kv_c`.
pub fn kv_append(
    ctx: &Ctx,
    kv_c: RaggedTensor,
    k_pe: Tensor,
...
mod mla_fa2 {
    ...
    const FILE: &str = "attn/attention_mla_fa2.cuh";
    ...
    /// The `::flashinfer::uint_fastdiv` this unit measures at 24 bytes
    /// (see the old `nvrtc-probes/attn_mla_params.py`).
    #[repr(C)]
    ...
    const fn at(base: u64, offset: i64) -> u64 {
        if offset < 0 {
            base
        } else {
    ...
    pub fn pack(
        ...
        MlaParams {
            ...
            ckv: layer.ckv_pages,
            kpe: layer.kpe_pages,
            partial_o: at(float_buf, info.partial_o_offset),
            partial_lse: at(float_buf, info.partial_lse_offset),
            final_o: buffers.out,
            final_lse: if want_lse { buffers.lse } else { 0 },
            q_indptr: at(int_buf, info.q_indptr_offset),
            kv_indptr: at(int_buf, info.kv_indptr_offset),
            partial_indptr: at(int_buf, info.partial_indptr_offset),
            merge_packed_offset_start: at(int_buf, info.merge_packed_offset_start_offset),
            merge_packed_offset_end: at(int_buf, info.merge_packed_offset_end_offset),
            merge_partial_packed_offset_start: at(
                int_buf,
                info.merge_partial_packed_offset_start_offset,
            ),
            merge_partial_packed_offset_end: at(
                int_buf,
                info.merge_partial_packed_offset_end_offset,
            ),
            merge_partial_stride: at(int_buf, info.merge_partial_stride_offset),
            kv_indices: buffers.kv_page_indices,
            q_len: at(int_buf, info.q_len_offset),
            kv_len: at(int_buf, info.kv_len_offset),
            q_start: at(int_buf, info.q_start_offset),
            kv_start: at(int_buf, info.kv_start_offset),
            kv_end: at(int_buf, info.kv_end_offset),
            work_indptr: at(int_buf, info.work_indptr_offset),
...
pub fn attention_decode(
    ...
    const OP: &str = "mla.attention_decode";
...
pub fn attention_prefill(
    ...
    const OP: &str = "mla.attention_prefill";
    dtype_dispatch!(OP, q.data.dtype, { Bf16 => () });
    let heads = stated(
        OP,
        nonzero(OP, "the head count this attention states", heads)?,
