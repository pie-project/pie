...
const FILE: &str = "attn/kv_paged.cuh";
...
/// Appends `k`/`v` rows into the pool's pages, addressed by the fire indptr
/// and the pool's page tables. Dispatches on the pool's storage: native
/// bf16 (with the envelope shadow when the scheme keeps one), or the
/// quantized writer the scheme byte names.
pub(crate) fn write_kv_to_pages(
    ...
    if native_bf16(pool) {
        write_kv_bf16(ctx, op, k, v, indptr, pool, kv_heads, head_dim)
    } else {
...
fn write_kv_bf16(
    ...
    let instantiation = if hnd {
        "::pie::attn::write_kv<::pie::true_type::value>"
    } else {
        "::pie::attn::write_kv<::pie::false_type::value>"
    };
    // The trailing null is the last-valid-page seat the appender does not
    // take on this path; first_token is 0 — every caller appends the whole
    // rectangle (the old skip-origin plumbing had no live user).
    ctx.fire(
        op,
...
fn write_kv_quantised(
    ...
    match scheme_of(op, pool)? {
        KvScheme::Fp8PerTensor => ctx.fire(
            op,
            Fire::at(FILE, "::pie::attn::write_kv_fp8_per_tensor")
        ...
        KvScheme::Fp4Block => {
            ...
            ctx.fire(
                op,
                Fire::at(FILE, "::pie::attn::write_kv_fp4_block").apply(Launch::grid(
...
/// pool. Shared by `mla.kv_append` and `index.kv_append` (which passes a
/// null rope plane of zero width).
#[allow(clippy::too_many_arguments)]
pub(crate) fn write_mla_to_pages(
    ...
    ctx.fire(
        op,
        Fire::at("attn/mla_paged.cuh", "::pie::attn::write_mla")
