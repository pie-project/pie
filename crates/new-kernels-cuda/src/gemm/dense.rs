...
//! heuristic, then remembers the winner on disk keyed by device and cuBLAS
//! version. All of it is selection, so all of it lives below the entry
//! (decision #13).
//!
//! `grouped_act_x_wt_bf16` and `act_x_wt_bf16_out_fp32` have no IR variant
//! in this wave; they stay because the moe and mla families fire them.
...
fn run_dense_tactic(
    ...
    match t.kind {
        GemmKind::Gemv => {
            ...
            gemv_bf16(&ctx, w, act, y, n, k, beta).is_ok()
        }
        GemmKind::Lt => {
