...
fn device_outputs(
    ...
    let program = plane
        ...
        .expect("the program compiles for the device");
    let extents = Extents {
        row_count: rows,
        sampled_rows: rows,
        query_len: rows,
    ...
    let wire_seeds: Vec<(u32, Vec<u8>)> =
        seeds.iter().map(|(c, v)| (*c, v.to_le_bytes())).collect();
    ...
    assert!(
        matches!(fired, Fired::Committed),
        "the device fire did not commit: {fired:?}"
    );
...
fn host_outputs(
    ...
    let inputs = PassInputs {
        logits: logits.map(|l| Value::F32(l.to_vec())),
        ..Default::default()
    };
    let stepped = reference
        .step(bound, &inputs, &mut NoKernels)
        .expect("the interpreter steps");
    assert!(
        stepped.committed,
        "the reference did not commit: {:?}",
        stepped.missed
    );
    channels
        .iter()
        .map(|&c| reference.host_take(bound, c).unwrap())
        .collect()
}
...
fn the_device_answers_what_the_interpreter_answers() {
    ...
    close(
        ...
        &f32s(&want[0]),
        1e-5,
        "gather_row of the scaled logits",
    );
    ...
    close(
        ...
        1e-3,
        "softmax row mass against one",
    );
    close(
        &wire_f32(&got[5]),
        &f32s(&want[5]),
        1e-5,
        "probability at the row's argmax (a row block's gather)",
    );
}
...
fn the_acceptance_rule_agrees_with_the_interpreter() {
    ...
    let previous: Vec<i32> = (0..ROWS as i32)
    ...
    assert_eq!(
        wire_bools(&got[0], ROWS as usize),
        bools(&want[0]),
        "accepted rows"
    );
    ...
    assert!(
        accepted > 0 && accepted < ROWS as usize,
        "the bound should split the rows, accepted {accepted}"
    );
}
...
fn the_gaussian_draw_and_the_transcendentals_agree_with_the_interpreter() {
    ...
    close(
        ...
        &vec![1.0; n],
        1e-5,
        "sin^2 + cos^2 against one",
    );
    close(
        &wire_f32(&got[6]),
        &vec![1.0; n],
        1e-5,
        "sqrt * rsqrt against one",
    );
    ...
    close(
        ...
        &want_root,
        1e-6,
        "sqrt against the host's",
    );

    let seed = eta_ir::rng::keyed_seed(key, counter);
    let want_noise: Vec<f32> = (0..n as u32)
        .map(|i| eta_ir::rng::hash_normal(seed, i))
        .collect();
    close(
        &wire_f32(&got[0]),
        &want_noise,
        1e-5,
        "the normal draw against `rng::hash_normal`",
    );
    ...
    assert!(
        variance > 0.4,
        "the device drew a near-constant plane (variance {variance})"
    );
}
