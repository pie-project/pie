...
pub const ENTRIES: &[EntryInfo] = &[
    ...
    entry("attention.short_conv", Reads::RowsAndLanes),
    entry("attention.sink", Reads::Rows),
    entry("attention.ssm_causal_conv1d", Reads::RowsAndLanes),
    entry("attention.ssm_gated_delta", Reads::RowsAndLanes),
    ...
    entry("linear.lora_correct", Reads::Rows),
