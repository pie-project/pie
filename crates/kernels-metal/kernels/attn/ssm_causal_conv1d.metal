...
template <typename T>
[[kernel]] void causal_conv1d_chunked_parallel(
    const device T* x              [[buffer(0)]],
    const device int* indptr       [[buffer(1)]],
    const device T* weight         [[buffer(2)]],
    const device float* conv_state [[buffer(3)]],
  ...
  const int first = int(pos.z) * 32;
  if (first >= span) return;
  for (int t = first; t < min(span, first + 32); ++t) {
    float acc = 0.0f;
    for (int k = 0; k < width; ++k) {
      const int src = t - (width - 1 - k) * dil;
      const float tap = (src < 0)
...
template [[host_name("causal_conv1d_chunked_parallel_bfloat16")]]
[[kernel]] void causal_conv1d_chunked_parallel<bfloat>(
    const device bfloat*, const device int*, const device bfloat*,
    const device float*, device float*, const device uint*, device bfloat*,
    const constant int&, const constant int&, const constant int&, uint3);
