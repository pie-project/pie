...
//!
//! The `abi` gate that remains is `Refusal::Unsupported` — a refusing
...
//! module's [`FACTS_ROWS`] and `crate::contract::HF_ROWS` are two tables
...
use crate::deployment::{AttnOutput, Deployment, KvStyle, LayerAttention, PrefillStyle, RecurrentShape, Refusal};
...
impl PlannedFamily for crate::gemma_2::forward::facts::Gemma2Facts {
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::gemma_2::forward::gemma2_cuda(self, class)
    }
    fn layers(&self) -> u32 {
        self.layers
    }
...
impl PlannedFamily
    for (
        crate::gpt_oss::forward::facts::GptOssFacts,
        crate::gpt_oss::forward::facts::GptOssCudaFacts,
    )
{
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::gpt_oss::forward::gpt_oss_cuda(&self.0, &self.1, class)
    }
    fn layers(&self) -> u32 {
        self.0.layers
    }
...
impl PlannedFamily
    for (
        crate::kimi_k2::forward::facts::KimiFacts,
        crate::kimi_k2::forward::facts::KimiCudaFacts,
    )
{
    fn unbuilt_kv_store(&self) -> Option<&'static str> {
    ...
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::kimi_k2::forward::kimi_cuda(&self.0, &self.1, class)
    }
    fn layers(&self) -> u32 {
        self.0.layers
    }
...
impl PlannedFamily for crate::kimi_k3::forward::facts::KimiK3Facts {
    fn unbuilt_kv_store(&self) -> Option<&'static str> {
        // Latent q/kv: the attention reads ckv and kpe planes, not the
        // paged k/v pair `kv_pools_for` builds, and no executor arm names
        // an MLA dispatch. `gpu::pools::mla_cache` is ported and waiting; until
    ...
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::kimi_k3::forward::kimi_k3_cuda(self, class)
    }
    fn layers(&self) -> u32 {
        self.layers
    }
...
impl PlannedFamily for crate::deepseek_v4::forward::facts::Dsv4Facts {
    fn unbuilt_kv_store(&self) -> Option<&'static str> {
        // Latent q/kv: the attention reads ckv and kpe planes, not the
        // paged k/v pair `kv_pools_for` builds, and no executor arm names
        // an MLA dispatch. `gpu::pools::mla_cache` is ported and waiting; until
    ...
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::deepseek_v4::forward::dsv4_cuda(self, class)
    }
    fn layers(&self) -> u32 {
        self.layers
    }
...
impl PlannedFamily for crate::nemotron_h::forward::facts::NemotronHFacts {
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::nemotron_h::forward::nemotron_h_cuda(self, class)
    }
    fn layers(&self) -> u32 {
...
impl PlannedFamily for crate::gemma3n::forward::facts::Gemma3nFacts {
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::gemma3n::forward::gemma3n_cuda(self, class)
    }
    fn layers(&self) -> u32 {
...
impl PlannedFamily
    for (
        crate::families::llama_like::forward::facts::LlamaLikeFacts,
        crate::families::llama_like::forward::facts::LlamaLikeCudaFacts,
    )
{
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::families::llama_like::forward::llama_like_cuda(&self.0, &self.1, class)
    }
    fn layers(&self) -> u32 {
        self.0.layers
    }
...
impl PlannedFamily
    for (
        crate::qwen_3_5::forward::facts::Qwen35HybridFacts,
        crate::qwen_3_5::forward::facts::Qwen35CudaFacts,
    )
{
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::qwen_3_5::forward::qwen3_5_hybrid_cuda(&self.0, &self.1, class)
    }
    fn layers(&self) -> u32 {
        self.0.layers
    }
...
impl PlannedFamily
    for (
        crate::gemma_4::forward::facts::Gemma4Facts,
        crate::gemma_4::forward::facts::Gemma4CudaFacts,
    )
{
    fn trace(&self, class: model_compiler::trace::FireClass) -> model_compiler::trace::ForwardPlan {
        crate::gemma_4::forward::gemma4_cuda(&self.0, &self.1, class)
    }
    fn layers(&self) -> u32 {
        self.0.layers
    }
...
fn gemma4_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
...
fn qwen35_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::qwen_3_5::forward::facts::{
...
type FactsFrom = fn(&Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal>;

/// One row per `model_type` this shell can OPEN.
///
/// A table rather than the chain of weight-name sniffs this replaces, for
/// exactly the reason `crate::contract::HF_ROWS` is a table: the supported
...
/// `crate::contract`'s header describes, and the same failure it names
...
/// C++ parse used.
fn gemma2_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::gemma_2::forward::facts::{Gemma2AttnFacts, Gemma2Facts};
    let hf = model.hf;
    let to_u32 = |v: i32| u32::try_from(v).unwrap_or(0);
...
fn gpt_oss_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
...
/// that ships one.
fn glm5_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::glm5::forward::facts::{Glm5DsaFacts, Glm5Facts, Glm5MlaFacts, Glm5MoeFacts};
    let hf = model.hf;
...
fn kimi_k2_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::kimi_k2::forward::facts::{KimiCudaFacts, KimiFacts, KimiMlaFacts, KimiMoeFacts};
    let hf = model.hf;
    let u = |v: i32| u32::try_from(v).unwrap_or(0);
    let facts = KimiFacts {
        layers: u(hf.num_hidden_layers),
    ...
    let cuda = KimiCudaFacts {
        ...
        rope_yarn_original: matches!(
            ...
            crate::config::schema::RopeScaling::OriginalYarn
        ),
    };
    Ok(Box::new((facts, cuda)))
}
...
/// `full_attn_at` states.
fn kimi_k3_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::kimi_k3::forward::facts::{
...
fn dsv4_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::deepseek_v4::forward::facts::{
...
fn nemotron_h_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::nemotron_h::forward::facts::{
...
fn gemma3n_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::gemma3n::forward::facts::{Gemma3nAltUpFacts, Gemma3nAttnFacts, Gemma3nFacts};
    let hf = model.hf;
    let u = |v: i32| u32::try_from(v).unwrap_or(0);
...
fn refuse_unservable_gqa(hf: &crate::config::schema::HfConfig) -> Result<(), Refusal> {
...
pub fn facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
...
fn llama_like_facts_from_hf(model: &Checkpoint<'_>) -> Result<Box<dyn PlannedFamily>, Refusal> {
    use crate::families::llama_like::forward::facts::{
...
// ── The value the driver actually gets ──

/// Derive a [`Deployment`] from a checkpoint, ONCE.
///
/// This is the whole point of §4 landing: the shell used to call
/// `facts_from_hf` from the admission of every fire, allocating a
/// `Box<dyn PlannedFamily>` and cloning per-layer `Vec`s each time —
/// while the lowering it feeds is cached precisely because it costs
/// 3.3 ms. The expensive answer was cached and its input was rederived.
///
/// The table above still exists and still switches on `model_type`,
/// which is correct: **somebody has to**, and this crate is where the
/// question is allowed to be asked. What changes is that the answer
/// crossing into a driver carries no family identity, so a driver
/// cannot ask it again.
///
/// # Errors
///
/// [`Refusal::Unsupported`] when no row matches.
pub fn deployment_from(model: &Checkpoint<'_>) -> Result<Deployment, Refusal> {
    let family = facts_from_hf(model)?;
    Ok(deployment_of(family.as_ref(), model))
}

/// The vtable's thirteen questions, asked once and written down.
///
/// A TRANSITIONAL SHAPE, and worth naming as such: the eleven
/// derivations should build a `Deployment` directly, and this function
/// exists so the boundary can land before they are rewritten. Every
/// line of it is a question the driver used to ask per fire.
#[must_use]
pub fn deployment_of(family: &dyn PlannedFamily, model: &Checkpoint<'_>) -> Deployment {
    let hf = model.hf;
    let layers = family.layers();
    let uniform = u32::try_from(hf.head_dim_kernel.max(hf.head_dim)).unwrap_or(0);
    let windows = family.window_by_layer(hf.sliding_window);
    let tables = family.tables(model);

    let attention = (0..layers)
        .map(|l| {
            let head_dim = family.head_dim_of(l, uniform);
            LayerAttention {
                head_dim,
                // A per-layer window table, or the one window repeated.
                // Repeated rather than special-cased: the `Vec` has no
                // opinion about which family produced it.
                window: windows.get(l as usize).copied().unwrap_or(hf.sliding_window),
                kv_source: family.kv_source(l).unwrap_or(l),
                sm_scale: family.sm_scale(head_dim),
                rope_theta: tables
                    .theta_by_layer
                    .get(l as usize)
                    .copied()
                    .unwrap_or(hf.rope_theta as f32),
                rotary_dim: tables.rotary_by_layer.get(l as usize).copied().unwrap_or(0),
            }
        })
        .collect();

    // THE KV SHAPE AS A VARIANT, which is what turns the MLA orphan
    // from a registry row that loads and dies into an arm a driver has
    // to write or refuse. `unbuilt_kv_store` was the vtable's way of
    // saying the same thing in a string.
    let kv = match family.unbuilt_kv_store() {
        None => KvStyle::Paged,
        Some(name) if name.contains("dsv4") || name.contains("compress") => {
            KvStyle::Dsv4 { ratios: Vec::new() }
        }
        Some(_) => KvStyle::Mla {
            kv_lora_rank: u32::try_from(hf.kv_lora_rank.max(0)).unwrap_or(0),
            qk_rope_head_dim: u32::try_from(hf.qk_rope_head_dim.max(0)).unwrap_or(0),
        },
    };

    let recurrent = family.gdn_shape().map(|g| RecurrentShape {
        linear_layers: g.linear_layers,
        conv_stride: g.conv_stride,
        state_stride: g.state_stride,
        state_elem: g.state_elem,
        k_h: g.k_h,
        v_h: g.v_h,
        k_d: g.k_d,
        v_d: g.v_d,
    ...
    Deployment {
        ...
        kv,
        recurrent,
        prefill: if family.planless_prefill() {
            PrefillStyle::Planless
        } else {
            PrefillStyle::Planned
        },
        attn_output: if family.pins_attention_values() {
            AttnOutput::DriverPinned
        } else {
            AttnOutput::StatedArgs
        },
        logit_softcap: tables.softcap,
        ple_dim: tables.ple_dim,
        // PRE unless the checkpoint ships the post-norm pair. The same
        // read the llama-like derivation makes, hoisted so a driver can
        // see it — an adapter's staging needs to name the projection
        // input, and which buffer that is depends on this.
        norm: if hf.model_type.starts_with("olmo") {
            crate::deployment::NormPlacement::Post
        } else {
            crate::deployment::NormPlacement::Pre
        },
