...
use crate::template::{
    ChatDecoder, GenericChatDecoder, Instruct, NoopToolDecoder, ReasoningDecoder, ThinkingDecoder,
    ToolDecoder,
};
            ...
            if let Ok(parsed) = serde_json::from_str::<serde_json::Value>(tool_json) {
                    ...
                    .and_then(|p| p.as_object())
                {
                    ...
                    for (pname, pspec) in props {
                        ...
                        prompt.push_str(&format!("{pname}{opt}: {ptype},\n"));
                    }
                    prompt.push_str("}) => any;\n\n");
                } else {
                    prompt.push_str("() => any;\n\n");
...
pub fn gpt_oss(tokenizer: Arc<Tokenizer>) -> Arc<dyn Instruct> {
    Arc::new(GptOssInstruct::new(tokenizer))
}
