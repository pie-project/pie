//! Write every kernel variant's expanded WGSL to a directory, one file per
//! entrypoint, with an `index.json` naming them — the input to a browser
//! page that hands each to Tint and reports what it refuses.
//!
//! `cargo run -p kernels-wgpu --example dump_wgsl -- <out-dir>`
...
/// What `engine-wgpu` does before compiling: every storage binding becomes
/// `read_write` (one buffer may be bound twice). Kept in sync by hand.
