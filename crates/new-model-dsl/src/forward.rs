...
use new_model_ir::{CacheRow, Dim, Dtype, GeomKind, Plan, Plane, RuntimeInput, Ty, ValueId};

use crate::record::{Recorder, Value, cache_name};
use crate::seam;
...
/// The bit-word side of a Facts struct; `#[derive(Facts)]` writes it.
pub trait FactWord {
...
/// The cache spaces a model declares, in the order `Plan::caches` will carry
/// them — kv rows and recurrent state slabs collect straight into `CacheRow`.
/// One spec serves attention-only models too: they simply never call `state`.
...
impl<F> Input<F> {
    ...
    /// One geometry vector of a cache space, as a declared input — the plan
    /// ops it feeds are pure functions of visible inputs (§7). The dims state
    ...
    /// indptr beside it.
    #[must_use]
    pub fn geometry(&self, cache: u32, kind: GeomKind) -> Value {
        let shape = match kind {
            GeomKind::Indptr => vec![Dim::LanesPlus(1)],
            GeomKind::Indices | GeomKind::SeqLens | GeomKind::LastPageLen => vec![Dim::Lanes],
        };
        self.rec.input(
    ...
    /// The index `geometry` wants, resolved from the name the model declared.
    #[must_use]
    pub fn cache_index(&self, name: &str) -> u32 {
        self.caches
            .rows
            .iter()
