...
use new_model_ir::{
    CacheRow, Cond, Def, Dim, Dtype, Node, Operands, Operation, Param, Plan, Plane, Repr,
...
impl Recorder {
    ...
    pub fn weight(&self, w: &Weight) -> ValueId {
        ...
        for plane in w.planes() {
            ...
            let index = intern(&mut p, name, plane.shape, shard, plane.repr);
    ...
    pub fn finish(self) -> Plan {
        let plan = Rc::try_unwrap(self.inner)
            ...
            .into_inner();
        if let Err(faults) = new_model_ir::check(&plan) {
...
fn intern(p: &mut Plan, name: String, shape: Vec<u64>, shard: Shard, repr: Repr) -> u32 {
    if let Some(i) = p.params.iter().position(|q| q.name == name) {
        let seen = &p.params[i];
        assert!(
            seen.shape == shape && seen.shard == shard && seen.repr == repr,
