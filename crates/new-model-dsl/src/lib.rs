...
//!
//! The old crate's `load.rs` (weight import rows) is deliberately absent for
//! now — deferred until `new-model` needs its loader; imports keep reading
//! the old crate meanwhile.
...
pub use facts::*;
pub use forward::*;
pub use new_model_dsl_macros::Facts;
pub use new_model_ir::{Dim, Dtype, GeomKind, Plan, Plane, Repr, RuntimeInput, Ty, ValueId};
