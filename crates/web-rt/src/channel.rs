//! crossbeam's channel surface for one thread.
//!
//! The runtime's scheduler and engine lanes are written against
//! `crossbeam::channel`: unbounded queues, `try_recv`, a blocking `recv`, a
//! `recv_timeout`, and a `Select` that waits for any of several receivers.
//! Here a send is a push and a wake; a blocking receive parks the calling
//! green thread (see [`crate::thread`]) until a sender wakes it. Outside a
//! green thread there is nothing to park, so blocking is a programming error
//! and panics with a message that says so.
...
/// Capacity is not enforced: on one thread a bounded send that had to wait
/// would deadlock, and every bounded channel the runtime makes is a one-shot
/// reply.
...
impl<T> Receiver<T> {
    ...
    /// Ready when a value or a disconnect is observable; otherwise registers
    /// the waker.
    ...
    /// Async receive, for callers that are tasks rather than green threads.
    ...
    /// Block the calling green thread until a value arrives or every sender
    /// is gone.
    ...
    /// Block the calling green thread for at most `timeout`.
    #[cfg(target_arch = "wasm32")]
    ...
    pub fn recv(&self) -> Result<T, RecvError> {
        unreachable!("web-rt channels only block on wasm32; native code uses crossbeam")
    }
...
/// crossbeam's `Select`, reduced to the one call the runtime makes: register
/// receivers, then `ready()` blocks until at least one has something to say.
...
/// crossbeam's `SegQueue` surface: a queue with no receivers, for pools.
