//! Bring the runtime up around one model and one WebGPU engine.
//!
//! This is `pie serve`'s boot sequence with the machine removed: the artifact
//! is bytes the page fetched (mounted at a virtual path so every path-based
//! loader keeps working) or bytes the page serves on demand (a lazy mount),
//! the engine is opened on a device the page already requested, and the
//! runtime's bootstrap runs with the config `translate.rs` would have
//! produced from a config file.
...
/// The few knobs a page states; everything else is derived or defaulted the
/// way the worker derives it. Parsed from a TOML document.
#[derive(Debug, Clone, Deserialize)]
#[serde(default, deny_unknown_fields)]
pub struct BootConfig {
    /// Name the SKU to skip identification.
    pub sku: Option<String>,
    /// `false` boots the sandbox without an engine: programs install and
    /// launch, and the first forward pass fails. For bring-up.
    pub engine: bool,
    ...
    /// What the device is taken to hold, in MiB, since WebGPU states no
    /// memory size; the engine assumes 8 GiB when unset. The weight budget
    /// is this times `gpu_mem_utilization`, less the KV and state pools.
    pub device_memory_mb: Option<u64>,
    pub power_preference: String,
    /// `wasm_max_memory_mb` for the sandbox's linear memories.
...
/// The virtual directory model artifacts are mounted under.
pub const MODELS_DIR: &str = "/models";

/// Where the artifact's bytes come from: `len` bytes the page serves on
/// demand through `fetch`, in windows (see `ztensor::memfs::mount_lazy`);
/// wasm memory holds only the mount's cache and the tensor being landed.
pub struct Mount {
    pub len: u64,
    pub fetch: Box<dyn ztensor::memfs::Fetch>,
}

/// `pie::model::Model::metadata` for an artifact: the checkpoint's own config
/// and the canonical tokenizer objects the import wrote beside the weights.
...
/// Mount the artifact, open the engine on the page's device, load the
/// weights, and bootstrap the runtime. One call per tab.
pub async fn boot(
    config: BootConfig,
    model_name: &str,
    mount: Mount,
    ...
    // Everything that reads the artifact runs on a green thread: the
    // mount's fetch parks the reader while the page does the I/O, and only
    // a green thread can park.
    let loaded = {
    ...
    // The weights are on the device and the metadata is lifted: the bytes
    // can go, which is most of the tab's memory. They go before the runtime
    // comes up so that its fibers' stacks take the freed low addresses:
    // wasm-bindgen's glue writes import results through signed 32-bit
    // offsets, so a stack above 2 GiB breaks every string-returning import.
    // A chunk still referenced elsewhere stays, and wasm memory never
    // shrinks either way. (The tokenizer was lifted into `metadata`; the
    // runtime opens the path only when that is missing.)
    ...
    // The runtime lives as long as the tab; nothing shuts it down.
...
/// Lift the metadata and load the engine (or, without a device, read the
/// serving stamp the runtime registers the model under).
fn load(
    config: &BootConfig,
    artifact: &Path,
    model_name: &str,
    ...
    let (engines, summary, model_id, kv_page_size) = match device {
        ...
        None => {
            // The runtime still registers a model; the artifact's serving
            // stamp names the SKU an engine would have traced.
...
/// The load thread's shadow stack: plan compilation and the engine's load
/// go deep, and a fiber stack is one heap allocation either way.
#[cfg(target_arch = "wasm32")]
const LOAD_STACK: usize = 8 << 20;

/// Run `f` on a green thread and await its result from the calling task.
#[cfg(target_arch = "wasm32")]
...
/// Natively there is no page to park for; the closure runs where it is.
...
/// Open the engine on the device, load the weights, and describe what came
/// up in the runtime's terms.
fn load_engine(
    config: &BootConfig,
    artifact: &Path,
    model_name: &str,
    ...
    // As many readout seats as the scheduler may have frames in flight: a
    // browser engine cannot wait for a seat, it can only refuse.
...
/// `translate.rs` for a tab: the bootstrap config a config file would have
/// produced, with the paths pointing at virtual locations.
fn runtime_config(
    config: &BootConfig,
    artifact: PathBuf,
    model_id: String,
