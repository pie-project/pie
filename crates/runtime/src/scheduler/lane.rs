//! The engine lane: one worker per engine, owning the engine, its channels
//! and the run-ahead peek, driven through an async API (`fire`, `control`)
//! that the scheduler awaits.
...
/// The lane's answer to one launch: the submission completion the scheduler
/// tracks until the frame settles, or why the lane refused it. Carried by a
/// oneshot inside the request (the async port's `fire()` reply) rather than
/// token-matched through the mailbox.
...
/// A control's answer: the commit the scheduler applies (was a ControlDone
/// reply matched by token through the mailbox).
...
/// Charges the time a request held the lane to its kind's counters on drop;
/// `None` for the requests that are not work (shutdown, landings).
struct LaneCharge<'a> {
...
enum LaneRequest {
    Launch {
        ...
        prefill: bool,
        /// Answered once the lane has submitted (or refused) the frame.
        reply: LaunchReplyTx,
    },
    Control {
        item: Box<QueuedItem>,
        /// Answered with the commit once the lane has executed the control.
        reply: ControlReplyTx,
    },
    Shutdown {
        response: crate::rt::channel::Sender<(Option<EngineBox>, ChannelJoin)>,
    },
    /// An asynchronously settled step has its rows: the host-side channels of
    /// its attachments can be pumped out now. A synchronously settling engine
    /// pumps right after `submit` and never posts this.
    /// Carries the outcome so the lane can publish the settlement itself,
    /// after `pump_out` has made the rows host-visible: a batch must not
    /// retire before the process can read what it produced.
    Landed {
...
impl LaneCommit {
    /// A control the lane could not carry out: its settling slot is dropped.
...
/// The engine, or the refusal every control answers with when there is none.
fn backend(engine: &mut Option<EngineBox>) -> Result<&mut EngineBox> {
    engine.as_mut().ok_or_else(|| anyhow!(NO_BACKEND))
}

/// Fail a request the lane will never serve, through the reply it carries.
/// The control's own callers hear the failure too: a copy's completion is
/// resolved, and dropping the item closes any RPC response it holds.
...
impl Lane {
    ...
    /// Fire one frame. The lane answers through the returned oneshot with the
    /// submission completion to track until it settles, or the refusal. The
    /// engine's run-ahead hint for the frame *after* this one is the lane's
    /// own concern: it peeks its queue (`fire_frame`).
    pub(super) fn fire(
        &self,
        submission: crate::engine::FrameFire,
        prefill: bool,
    ...
    /// Execute one control. The lane answers through the returned oneshot
    /// with the commit the scheduler applies.
    ...
    fn next_request(
        ...
        // A tab has one thread: spinning here would starve the scheduler
        // that is about to post the next request, so the lane parks at once.
    ...
    fn run(
        ...
        // Per asynchronously settling frame, each step's attachment instances,
        // for the pump-out its landing asks for.
        ...
        if let Some(engine) = engine.as_mut() {
            ...
            if engine.settles_asynchronously() {
                // The sink only reports the landing; the lane publishes the
                // settlement once the rows are pumped out (see `Landed`).
        ...
        loop {
            ...
            // Each request's reply stays outside the unwind boundary, so a
            // panic mid-request is still answered with a reason — and, for a
            // control, with the commit its kind owes the scheduler regardless.
            let panicked = match request {
                ...
                LaneRequest::Landed {
                    ...
                    // Rows are host-visible now: settle, which wakes the
                    // scheduler's park and, through it, the process.
    ...
    /// The commit a control still owes the scheduler when the lane cannot
    /// execute it: a bind's join hold is released, a copy's settling slot is
    /// dropped, and nothing is bound or copied.
    ...
    fn fire_frame(
        ...
        if asynchronous {
            // The rows are not back yet, so the host-side channels have nothing
            // to take now; the landing posts `LaneRequest::Landed` and the pump
            // runs then. Adopted (device-mirrored) channels are woken at settle.
            landed.insert(
                ticket.id,
                submitted
                    .steps
    ...
    /// Bind an instance and join its channels: the bound instance the
    /// scheduler will track.
    fn bind(
        engine: &mut EngineBox,
        channels: &mut ChannelJoin,
    ...
    /// A standalone copy: its RPC caller gets the completion (or the
    /// refusal), and the scheduler tracks the same completion in the
    /// control's settling slot.
    fn answer_copy(
