...
/// Post one item to every live scheduler.
...
/// Post a fenced pipeline leave to every live scheduler; each fence resolves
/// once that scheduler has processed the leave.
...
pub(crate) fn wave_trace_emit(line: String) {
    ...
    let buffered = *BUFFERED.get_or_init(|| {
        // A tab has no thread to flush from; the trace stays unbuffered there.
...
// Async-port migration (slice 0): the scheduler main loop is an async task,
// so its `SchedulerItem` mailbox is a tokio unbounded channel — senders (the
// server, processes, the engine lane thread) `send` synchronously, the task
// `recv().await`s. The engine lane's own launch/control queues stay crossbeam.
type SchedTx = tokio::sync::mpsc::UnboundedSender<SchedulerItem>;
type SchedRx = tokio::sync::mpsc::UnboundedReceiver<SchedulerItem>;
/// Every answer the lane still owes, polled as one set: a future per posted
/// launch or control. The lane serves its launch queue in order, so a launch
/// answer belongs to the oldest posted batch; a control answer carries the id
/// of its settling slot (the lane never sees these ids).
#[derive(Default)]
struct LaneReplies {
    next_control_id: u64,
...
impl std::future::Future for ReplyWait {
    ...
    /// A dropped sender means the lane went away without answering; that
    /// fails the request.
    fn poll(
...
impl BatchScheduler {
    ...
    async fn run(
        ...
        loop {
            ...
            let item = if pending.is_empty()
                && in_flight_launches.is_empty()
                && in_flight_control.is_empty()
                // A posted control that has not answered yet (bind, close,
                // register: the kinds without a settling slot) must keep the
                // loop in the selecting park below, or its answer goes unheard.
                && replies.is_empty()
                && !stopping
            {
                match rx.recv().await {
                ...
                // Settlements are futures: the front accepted launch's and every
                // ready control's completion join the park below, so a landing
                // wakes this loop through its own waker rather than a nudge
                // through the mailbox. A settled or failed one retires first.
                let front_launch =
                    in_flight_launches
                        .front()
                ...
                // Wake on the mailbox, any answer the lane owes, or a
                // settlement, whichever lands first. The answer is applied
                // right after the select so it is never dropped.
                ...
                // `tokio::select!` evaluates every branch's future expression
                // even when its `if` precondition is false (it only skips
                // polling), so each expression must be total: `pending()`
                // when there is nothing to wait on. The inner block ends the
                // futures' borrows before the answers are applied below.
                let parked = {
    ...
    /// A launch the lane has answered: the completion to track until it
    /// settles, or the refusal. (Was the token-matched `LaunchDone` reply.)
    ...
    /// Apply one answer from the lane: a launch answer goes to the oldest
    /// posted batch (the lane answers launches in posting order), a control
    /// answer to the slot its id names.
    fn apply_lane_reply(
        reply: LaneReply,
        in_flight_launches: &mut VecDeque<PendingLaunchBatch>,
        instances: &mut HashMap<u64, TrackedInstance>,
