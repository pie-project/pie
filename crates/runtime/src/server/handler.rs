...
impl Session {
    pub(super) async fn handle_launch_process(
        ...
        // A runtime booted without an engine (a bring-up mode) has no KV
        // store for a process to bind; the store accessors below assume one,
        // so refuse here by name rather than panic inside the guest's first
        // host call.
