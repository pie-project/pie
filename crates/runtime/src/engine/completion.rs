...
/// The address of a `TerminalCell` owned by a `WorkItemCompletion` the
/// scheduler holds for as long as anything carries this pointer, or null.
/// Every access through it is an atomic store (`settle`), so the address may
/// travel between threads: the scheduler's one `unsafe impl Send`.
