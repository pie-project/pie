...
use new_kernels_metal::mla::MlaPlan;
use new_kernels_metal::{Ctx, DecodePlan, KvPool, PrefillPlan, RaggedTensor, RecurrentPool, Tensor};
use new_model_ir::{Def, GeomKind, RuntimeInput, ValueDecl, ValueId};

/// Loader-resolved weights, one row per `Plan::params` entry —
/// `Def::Weight(i)` resolves to row `i`. `None` marks a param the shell has
/// not bound as one dense handle (an mxfp4 bank is two planes and fits no
/// row here — see the `MENLO-SEAM` at `moe.matmul_select_bias`); resolving
/// such a row is a binding bug and panics.
#[derive(Clone, Debug, Default)]
pub struct WeightTable(pub Vec<Option<Tensor>>);
...
/// in [`FireBindings::positions`]; these are the rest. No op names them —
/// `attention.plan_*` names kv geometry instead — so the plan-building arms
/// bind them from here. This is the driver side of the seam the
/// `MENLO-SEAM` markers in `new_kernels_metal::attn` describe.
#[derive(Clone, Copy, Debug)]
...
pub struct FireBindings {
    ...
    /// `RuntimeInput::Geometry { cache, kind }` routes to row `cache`.
...
impl<'c> Run<'c> {
    ...
    /// arriving here is a dispatch-arm bug, answered with a panic.
        ...
        match &self.values[at].def {
            ...
            Def::Weight(w) => {
                ...
                self.weights
                    ...
                    .flatten()
                    .unwrap_or_else(|| {
                        panic!("value {at} is weight {row}, which the shell has not bound")
                    })
            }
