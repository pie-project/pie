...
impl Walk<'_, '_> {
    ...
    fn read_file(
        ...
        // A path mounted in memory (a browser tab has no file behind it) is
        // copied straight out: no handle, no read fan-out, nothing to release.
    ...
    fn decode_bytes(
        ...
        // One worker runs inline: a single-threaded platform cannot spawn.
        if workers <= 1 {
