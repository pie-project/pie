...
/// `ztensor_compat::index`, except that a path mounted in memory opens as
/// the `.zt` it is: the format sniff reads the head of a file, and a chunked
/// mount has no file (nor a single allocation) behind it.
