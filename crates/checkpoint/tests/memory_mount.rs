...
/// Two small tensors, then `wide` more of 3000 floats each: with a mount
/// chunked at a few KiB most of those lie across a seam.
...
/// A lazy mount over the real file: every read the loaders make is served
/// by a fetch callback, with the default 8 MiB window (the whole artifact
/// fits one window, so the cache answers everything after the first fetch)
/// and then with a window of a few KiB, so tensors straddle windows and the
/// wide ones are fetched straight through.
fn a_lazy_mount_reads_like_the_file() {
    ...
    for window in [ztensor::memfs::LAZY_WINDOW, 4096] {
        // The fetch reads the file itself, and counts what it served.
        ...
        // No tensor has a zero-copy view; all of them read back whole.
        ...
        if window == ztensor::memfs::LAZY_WINDOW {
            ...
            // Every span the loader read wider than a window came straight
            // through in one aligned request.
            assert!(served.iter().any(|(_, n)| *n > window), "{served:?}");
            // The mount holds a bounded cache, never the artifact.
        ...
        // A fetch that fails refuses the read and says why.
...
fn a_single_chunk_mount_reads_like_the_file() {
    ...
    // Metadata read through the mount matches the file, path aside.
    let from_file = parse_metadata(&real).unwrap();
    let from_mount = parse_metadata(fake).unwrap();
    assert_eq!(from_mount.files.len(), 1);
    assert_eq!(from_mount.files[0].path, fake.display().to_string());
    ...
    // A plan compiled against the mount executes off the mounted bytes and
    // lands the same tensors the file does.
    let plan =
    ...
    // A mount of the wrong length is refused where a short file would be.
...
/// The artifact mounted as 4 KiB chunks, so most of its 12 KB tensors lie
/// across a seam: every reader lands the bytes the file does.
fn a_chunked_mount_reads_like_the_file() {
    ...
    // The seams are where they should be: some tensors have no zero-copy
    // view, and all of them read back whole.
    let source = ztensor::Source::open(fake).unwrap();
    ...
    // A mount short of the declared length is refused, chunked or not.
    ...
    assert!(verify_declared_files(&plan, Path::new("/")).is_err());
    // And a read past a mount's end is refused, not zero-filled.
