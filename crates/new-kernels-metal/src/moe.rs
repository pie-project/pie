...
//! [`quant`](crate::quant) has no routed point here.
...
pub fn topk_softmax(
    ...
    nonzero(OP, "rows", logits.rows)?;
    ctx.fire(
        Fire::at("moe/route.metal", entry).apply(Grid::of([lanes, logits.rows, 1], [lanes, 1, 1])),
        &[
            logits.arg(),
            routes.arg_mut(),
            weights.arg_mut(),
...
pub fn topk_sigmoid(
    ...
    ctx.fire(
        Fire::at("moe/route.metal", entry).apply(grid),
        &[
            logits.arg(),
            routes.arg_mut(),
            weights.arg_mut(),
...
pub fn matmul_select(
    ...
    ctx.fire(
        Fire::at("moe/select.metal", entry).apply(select_gemv_grid(OP, y.width, y.rows)?),
        &[
            x.arg(),
            bank.arg(),
            routes.arg(),
...
pub fn matmul_select_bias(
    ...
    ctx.fire(
        Fire::at("quant/qmv.metal", entry).apply(Grid::of(
