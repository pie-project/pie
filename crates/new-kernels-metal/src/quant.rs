...
//! instantiation stamps, and their grids. No IR variant lands here directly;
//! the moe and quantized-gemm entries select these points and fire them.
//!
...
pub fn qmm_grid(
    ...
    if n % bn != 0 {
        return Err(refuse(
            op,
            format!(
                "the column count is {n}, not a multiple of {bn}: `qmm_t.metal` \
