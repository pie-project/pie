...
impl Options {
    ...
    /// Open an artifact whose bytes are already in memory — one `Arc<[u8]>`
    /// or a `Vec` of chunks laid end to end; `name` is what the source calls
    /// itself and what any shard paths are resolved beside.
    pub fn from_memory(
        self,
...
impl<'a> Tensor<'a> {
    ...
    /// The zero-copy view: the mapped file, or the chunk of a memory mount
    /// the bytes lie in. Bytes straddling two chunks have no such view
    /// (`caps().map` says so); `bytes` reads them either way.
