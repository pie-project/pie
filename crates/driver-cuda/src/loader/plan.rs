...
///
/// The same three as Metal's. The mask is bounded by what has a HOST
/// implementation ([`CONVERT_TILE_MAP_MASK`](model_loader::plan::CONVERT_TILE_MAP_MASK)),
/// because the host path is the fallback and the reference — see
/// `every_transform_this_driver_claims_has_a_host_implementation`. Which of
/// them actually run on the device is [`super::arena::DeviceArena`]'s answer,
/// given per load through `ArenaBacking::tile_map_caps`, and is `Cast` and
/// `Scale` today.
///
...
mod tests {
    ...
    fn every_transform_this_driver_claims_has_a_host_implementation() {
        // The host executor is the FALLBACK and the REFERENCE, which is a
        // stronger reason to hold this than the one it started with.
        //
        // It used to mean "this driver ports no transcode loops, so the host
        // had better run everything". The arena backing now runs `Cast` and
        // `Scale` on the device (`loader::arena`), so that is no longer why.
        // What the property buys today is that turning the device path off --
        // `PIE_LOADER_DEVICE_TRANSFORMS=0`, a build without `bridge`, or a
        // kernel that refuses an operand shape -- always lands somewhere that
        // can finish the load. It is also what lets the same plan be executed
        // into a host arena and a device arena and the bytes compared.
        assert_eq!(
            CUDA_TILE_MAP_MASK & !model_loader::plan::CONVERT_TILE_MAP_MASK,
            0,
            "a transform with no host implementation leaves the load with no \
             fallback and no reference to check the device answer against"
        );
    }
