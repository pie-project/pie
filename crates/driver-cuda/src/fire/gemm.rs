...
//! [`crate::bind::service`], §45's continuation took the quantized router
//! into [`crate::bind::quant_gemm`], and this module takes what was left:
...
//! **driver-owned launch is not a row**: [`crate::fire::gemv::gemv_bf16`] is
//! Rust, its two `__global__`s are the `gemm/gemv` JIT unit, and its refusal
//! is a type — [`crate::fire::gemv::Gemv::Declined`] carrying *which* of the
...
//! the C++ and after [`crate::fire::flashinfer_moe`]'s — and it is a second
...
//! the crate's `cudarc` feature list for [`crate::bind::quant_gemm`] already;
...
use crate::fire::gemv::{Gemv, gemv_bf16};
...
/// function returns a `bool` and [`crate::fire::gemv::gemv_bf16`] does not:**
...
/// and in [`crate::fire::flashinfer_moe`], and nowhere else. The line is
/// quoted rather than cited: `h ^= v + 0x9e3779b97f4a7c15 + (h << 6) + (h >>
/// 2)`, boost's `hash_combine` over the golden-ratio word.
///
/// Spelled here rather than reused from [`crate::fire::flashinfer_moe`]
...
/// so this and [`crate::fire::flashinfer_moe`]'s copy are the only remaining
