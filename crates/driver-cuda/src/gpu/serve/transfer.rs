...
///
/// The host legs go through `gpu::pools::swap_pool` and `layout::swap_plan`,
...
pub fn pie_cuda_copy_kv(
    ...
    guard("pie_cuda_copy_kv", PIE_STATUS_DRIVER_ERROR, move || {
        ...
        if host_src || host_dst {
            ...
            let plan = crate::gpu::pools::swap_pool::SwapPoolLayout::for_cache(
                &device_buffers,
            ...
            if !reusable {
                ...
                let st = plan.streams();
                let mk = |want: bool| -> Option<crate::gpu::device::OwnedStream> {
                    want.then(|| crate::gpu::device::OwnedStream::new(0).ok()).flatten()
                };
        ...
        let stream = match crate::gpu::device::OwnedStream::new(0) {
            Ok(s) => s,
        ...
        .map_or_else(|| stream.as_ref(), crate::gpu::device::OwnedStream::as_ref);
        for op in plan.ops() {
        ...
        if !cells.is_empty() {
            ...
            let up = |vals: &[u32]| -> Result<crate::gpu::device::DeviceBuffer, i32> {
            ...
            for i in 0..kv_ref.layers() {
                ...
                let layer = crate::gpu::bind::abi::KvCacheLayerView {
                    layer: i as i32,
                    ...
                    head_dim: d,
                    scheme: crate::gpu::bind::abi::KvCacheScheme::Native,
                ...
                unsafe {
                    crate::gpu::bind::abi::ffi::pie_k_attn_copy_kv_cells_bf16(
                        layer,
                        d_dp.as_ptr().cast(),
                        d_doff.as_ptr().cast(),
                        d_sp.as_ptr().cast(),
...
pub fn pie_cuda_copy_state(
    ...
    guard("pie_cuda_copy_state", PIE_STATUS_DRIVER_ERROR, move || {
        ...
        let Ok(stream) = crate::gpu::device::OwnedStream::new(0) else {
            return PIE_STATUS_DRIVER_ERROR;
        };
        let alloc = crate::gpu::device::Allocator::new();
        let need = ranges
...
pub fn pie_cuda_resize_pool(
    ...
    guard("pie_cuda_resize_pool", PIE_STATUS_DRIVER_ERROR, move || {
        ...
        let layout = match old.as_ref().map(|o| o.cache.layout().with_num_pages(target as i32)) {
            ...
            None => {
                let per = crate::gpu::pools::kv_cache::PerLayer {
                    head_dim: (0..n_layers)
                        .map(|i| {
                            if !is_gemma4 {
                ...
                match crate::gpu::pools::kv_cache::KvCacheLayout::plan_per_layer(
                    n_layers as i32,
                    target as i32,
                    page_size as i32,
                    kv_heads,
        ...
        let Ok(cache) = crate::gpu::pools::kv_cache_live::KvCache::materialize(layout, &mut ops) else {
            return PIE_STATUS_EXHAUSTED;
        };
        ...
        crate::gpu::serve::state::install_kv(&mut state.kv, &mut state.fire_arrays.epoch, fresh);
        std::sync::atomic::fence(std::sync::atomic::Ordering::Release);
        if let Some(notify) = state.notify {
            unsafe { notify(state.notify_ctx, completion.wait_id, completion.target_epoch) };
        }
