...
use crate::gpu::fire::launch::sg_trace;
use super::checked;
...
pub(crate) fn create_impl(
    ...
    if let Err(e) = crate::gpu::device::Device::bind(device_ordinal) {
        eprintln!(
    ...
    let boxed = Box::new(Shell {
        ...
        supergraph: crate::gpu::fire::recordings::Recordings::new(),
        kv: None,
        gdn: None,
        channels: std::collections::BTreeMap::new(),
        swap: None,
        lora_arena: crate::gpu::fire::lora::LoraStageArena::default(),
        scratch: None,
        fire_stream: None,
        in_flight: std::collections::VecDeque::new(),
        fire_alloc: None,
        ptir: crate::gpu::program::Runtime::default(),
        ptir_control: None,
        ptir_sessions: std::collections::BTreeMap::new(),
        ptir_programs: crate::gpu::program::Programs::new(),
...
pub(crate) fn destroy_impl(driver: *mut PieDriver) {
    if !driver.is_null() {
        ...
        if let Some(mut h) = shell.cublas.take() {
            h.release(&mut crate::gpu::device::cublas::LiveCublas);
        }
        if let Some(mut scratch) = shell.scratch.take() {
            let mut sops = crate::gpu::fire::attention_workspace::LiveStagingOps;
...
pub(crate) fn load_impl(state: &mut Shell, snapshot: &std::path::Path) -> Result<(), i32> {
    ...
    let target = crate::gpu::weights::plan::cuda_storage_target(state.tp_rank, state.tp_size);
    let (plan, _moe) =
        crate::gpu::weights::plan::compile_load_plan(snapshot, &meta, &target, &descriptor_json)
            .map_err(|e| {
                eprintln!("[driver-cuda] load_model: {e}");
                PIE_STATUS_UNSUPPORTED
            })?;
    let alloc = crate::gpu::device::Allocator::new();
    let staged = crate::gpu::weights::stage::stage_plan_weights(&plan, snapshot, &alloc).map_err(
        |e| {
    ...
    match &model.deployment.kv {
        model::deployment::KvStyle::Paged => {}
        model::deployment::KvStyle::Mla { .. } => {
            eprintln!(
                "[driver-cuda] load_model: this checkpoint attends through a \
                 latent ckv/kpe pair, which this driver does not build — \
                 `gpu::pools::mla_cache` is ported and has no forward path to \
                 serve. Refusing here rather than at the first fire."
            );
            return Err(PIE_STATUS_UNSUPPORTED);
        }
        model::deployment::KvStyle::Dsv4 { .. } => {
            eprintln!(
                "[driver-cuda] load_model: this checkpoint attends through a \
                 compressed KV plane, which this driver does not build — \
                 `gpu::pools::dsv4_compress_cache` is ported and has no forward \
...
fn calibrate_planner(state: &mut Shell) {
    ...
    use crate::gpu::serve::state::{InstanceEntry, ProgramEntry};
...
fn synthetic_fire(
    ...
    crate::gpu::fire::launch::step_impl(state, &frame, &step, None)
}
...
fn wire_trace_names(model: &mut LoadedModel) {
    ...
    for (trace, parts) in wiring.joins {
        ...
        if abut {
            model.weights.insert(trace, crate::gpu::weights::stage::WeightSpan {
                ptr: spans[0].ptr,
    ...
    model.gemma_layer_scalars = wiring
        ...
        .map(|n| {
            model.weights.get(n).map_or(1.0f32, |b| {
                match crate::gpu::weights::stage::read_span(*b) {
...
fn capabilities_json(state: &mut Shell, snapshot: &std::path::Path) -> Result<Vec<u8>, i32> {
    ...
    let device =
        crate::gpu::device::Device::bind(state.device_ordinal)?;
...
pub(crate) fn ptir_target(ordinal: i32) -> Result<crate::gpu::program::Target, i32> {
    let device = crate::gpu::device::Device::bind(ordinal).map_err(|error| {
    ...
    let nvrtc = crate::gpu::program::compile::version().map_err(|error| {
        eprintln!("[driver-cuda] register_program: {error}");
        PIE_STATUS_DRIVER_ERROR
    })?;
    Ok(crate::gpu::program::Target {
        major,
        minor,
