...
impl DecodePlan {
    ...
    #[must_use]
    pub fn new() -> Self {
        let cache = unsafe { crate::gpu::bind::abi::ffi::pie_x_make_decode_plan() };
    ...
    pub fn set_int_base(&mut self, bytes: usize) {
        unsafe { crate::gpu::bind::abi::ffi::pie_x_set_decode_plan_int_base(self.cache, bytes) };
    }
    ...
    pub fn plan_decode_variant(
        ...
        unsafe {
            crate::gpu::bind::abi::ffi::pie_x_plan_attention_flashinfer_decode_bf16(
                self.cache,
...
impl Drop for DecodePlan {
    fn drop(&mut self) {
        unsafe { crate::gpu::bind::abi::ffi::pie_x_destroy_decode_plan(self.cache) };
    }
}
...
impl PrefillPlan {
    ...
    #[must_use]
    pub fn new() -> Self {
        let cache = unsafe { crate::gpu::bind::abi::ffi::pie_x_make_prefill_plan() };
    ...
    pub fn plan_prefill(
        ...
        unsafe {
            crate::gpu::bind::abi::ffi::pie_x_plan_attention_flashinfer_prefill_bf16(
                self.cache,
                qo_indptr_h.as_ptr(),
                kv_page_indptr_h.as_ptr(),
...
impl Drop for PrefillPlan {
    fn drop(&mut self) {
        unsafe { crate::gpu::bind::abi::ffi::pie_x_destroy_prefill_plan(self.cache) };
    }
}
...
pub struct DispatchCtx {
    ...
    /// different splits. See [`crate::gpu::device::PeelWindowWord`].
    ...
    pub lora: Option<(*const crate::gpu::fire::lora::LoraFireState, *mut c_void)>,
...
pub struct AttnCtx {
    ...
    pub workspace: crate::gpu::bind::abi::AttentionWorkspaceView,
    ...
    /// raised in.
    pub prefill_workspace: crate::gpu::bind::abi::AttentionWorkspaceView,
    /// One KV view per layer, indexed by the launch's layer.
    pub layers: Vec<crate::gpu::bind::abi::KvCacheLayerView>,
...
fn dispatch_generated<R: Resolver>(
    ...
    fn kv_view(a: Option<&AttnCtx>, layer: usize) -> crate::gpu::bind::abi::KvCacheLayerView {
        a_of(a).layers[layer]
    }
    ...
    if spec.per_head_dim.is_some() || spec.rope_partial.is_some() {
        return false;
    }
    ...
    match bound.kernel {
        ...
        "attn::dispatch_attention_flashinfer_prefill_capture_bf16" => {
            ...
            let layer = a
                ...
                .get(bound.layers.start as usize)
                .ok_or_else(|| DispatchRefusal::NoAttnCtx(bound.kernel.to_string()))?;
            // The PLAN first: a null sink is a wrong answer, a null plan
            // is a null dereference inside the launcher. Under `Union`
            // this arm is recorded on a fire that raised no prefill plan
            // as a matter of course.
            if a.prefill_plan.is_null() {
                return Err(DispatchRefusal::NoAttnCtx(format!(
                    "{}: the fire raised no prefill plan",
                    bound.kernel
                )));
            }
            if a.score_out.is_null() || a.score_indptr_d.is_null() {
            ...
            unsafe {
                ffi::pie_k_attn_dispatch_attention_flashinfer_prefill_capture_bf16(
                    ...
                    // `attn_score` is for.
                    i32::try_from(a.score_window).unwrap_or(i32::MAX),
                    a.logits_soft_cap,
                    a.sm_scale,
                    a.lse_out_d,
                );
        ...
        "attn::dispatch_attention_flashinfer_prefill_bf16" => {
            ...
            let layer = a
                ...
                .get(bound.layers.start as usize)
                .ok_or_else(|| DispatchRefusal::NoAttnCtx(bound.kernel.to_string()))?;
            // A null plan is a null dereference INSIDE the launcher --
            // a host segfault with no CUDA error and no stack. It became
            // reachable when the window class turned into a guard
            // (`.wiki/driver/graph.md` §4.1): a decode fire's lowering now
            // CONTAINS the prefill arm, and under `Union` that arm is
            // recorded whether the fire raises a prefill plan or not.
            if a.prefill_plan.is_null() {
                return Err(DispatchRefusal::NoAttnCtx(format!(
                    "{}: the fire raised no prefill plan",
                    bound.kernel
        ...
        "attn::dispatch_attention_flashinfer_prefill_custom" => {
            ...
            let layer = a
                ...
                .get(bound.layers.start as usize)
                .ok_or_else(|| DispatchRefusal::NoAttnCtx(bound.kernel.to_string()))?;
            // THREE THINGS THIS ARM CANNOT RUN WITHOUT, and the plan is
            // the one that kills the process rather than returning.
            //
            // A null mask is a wrong answer; a null PLAN is a null
            // dereference inside the launcher, which is a host segfault
            // with no CUDA error and no stack. Under `GuardMode::Union`
            // this arm is RECORDED on every fire whether or not the fire
            // takes it, so "the fire happens to be a decode and raised no
            // prefill plan" is a routine state and not a bug -- it has to
            // be answered here, before the launcher, exactly as the
            // score-capturing arms answer their own missing sink.
            if a.prefill_plan.is_null() {
                return Err(DispatchRefusal::NoAttnCtx(format!(
                    "{}: the fire raised no prefill plan",
                    bound.kernel
        ...
        // TWO ARITIES, and only one of them generates.
        //
        // `[x, gate, out]` — the lowering assigned a distinct destination
        // — is the row's shape and the generated branch serves it,
        // staging the copy the aliasing needs. `[x, gate]` states no
        // output at all, so the generated guard's `n_out >= 1` correctly
        // declines and lands here. Which argument is the destination is
        // the difference, and a row states one answer.
        ...
        // head) over the trailing head width, weight fp32-widened. Not
        // generatable: its row count is `rows * gdn.v_h`, a PRODUCT of
        // the fire's rows and a context field, and no `Source::` spells
        // one. See the row's own comment.
        ...
        // Not generatable: `B` is one operand's row width over another's,
        // and every `*WidthOver` divides by a context field.
...
pub fn run_captured<R: Resolver>(
    ...
    attn: AttnRegions<'_>,
    gdn: Option<&GdnCtx>,
    builder: &mut crate::gpu::device::SupergraphBuilder<'_>,
) -> Result<usize, RunRefusal> {
