...
use crate::gpu::device::stream::StreamRef;
...
/// build targets, and [`crate::gpu::device::Device::bind`] refuses to start when the
...
impl Graph {
    ...
    /// Produced by [`crate::gpu::device::CaptureScope::end`], which is the only
...
impl GraphExec {
    ...
    /// [`crate::gpu::fire::recordings::BucketKey`]. Grid and block dims are
...
// — S2 of the supergraph ladder, and the piece [`crate::gpu::device::graph`]'s
...
/// runtime.
///
/// The same major-version split [`crate::gpu::device::graph`]'s `add_node`
...
/// stream pool.
///
/// The root stream must already be inside a capture — [`crate::gpu::device::CaptureScope`]
