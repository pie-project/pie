...
fn build_lowering(
    family: &dyn crate::facts::PlannedFamily,
    class: model_compiler::trace::FireClass,
...
fn admit(
...
) -> Result<(Admitted, Box<dyn crate::facts::PlannedFamily>), i32> {
    use model_compiler::trace::FireClass;
    ...
    let family = crate::facts::facts_from_hf(&model.checkpoint())?;
    ...
    if fire_rows.iter().any(|r| r.lora) {
        eprintln!(
            "[driver-cuda] launch: a region carries an adapter \
    ...
    if let Some(shape) = family.gdn_shape() {
        ...
        if (*gdn).is_none() {
            ...
            let is_linear: Vec<bool> =
                (0..shape.layers).map(|l| shape.linear_layers.contains(&l)).collect();
            let cache = crate::store::recurrent_state_cache::RecurrentStateCache::allocate_bf16_recurrent(
                &is_linear,
        ...
        if gdn_state.ensure_slots(need_slots, &alloc, &stream)? {
            *epoch += 1;
        }
        ...
        if gdn_state.ensure_slots(need_buffer.max(need_slots), &alloc, &stream)? {
            *epoch += 1;
        }
...
struct FireInputs {
    ...
    layers: Vec<crate::launch::KvCacheLayerView>,
    ...
    d_valid: crate::cuda::DeviceBuffer,
}
...
fn kv_and_arrays(
    kv: &mut Option<KvState>,
    fire_arrays: &mut crate::serve::state::FireArrays,
    format: crate::store::KvCacheFormat,
    family: &dyn crate::facts::PlannedFamily,
    model: &LoadedModel,
    alloc: &crate::cuda::Allocator,
...
struct AttnPlans {
    ...
    prefill_workspace: crate::launch::AttentionWorkspaceView,
...
fn raise_attn_plans(
    ...
    use crate::model::executor::{DecodePlan, PrefillPlan};
    ...
    // unraised.
    if !planless_prefill {
        prefill_ws.begin_plan_update(&mut sops).map_err(|_| PIE_STATUS_DRIVER_ERROR)?;
...
fn publish_seam_pins(
    ...
    let staged = (step.has_user_mask != 0).then(|| {
        crate::model::page_mask::element_mask::from_words(
            qo_indptr,
            kv_indptr,
            kv_lens,
            page_size,
    ...
    let element_mask = match staged {
        Some(r) => Some(r?),
        None => crate::model::page_mask::element_mask::plan_causal(
            qo_indptr,
            kv_indptr,
            kv_lens,
            page_size,
...
fn attention_pins(
    ...
    if !family.pins_attention_values() {
        return Ok((None, None));
    }
...
struct SamplingSites<'a> {
    ...
    programs: &'a crate::ptir::Programs,
    control: &'a mut Option<crate::ptir::Control>,
    sessions: &'a mut std::collections::BTreeMap<u64, crate::ptir::session::Session>,
    disk: &'a crate::ptir::Disk,
    device_ordinal: i32,
    named_bufs: &'a std::collections::BTreeMap<model_compiler::trace::ValueId, crate::cuda::DeviceBuffer>,
}
...
pub(crate) fn step_impl(
    ...
    use crate::model::executor::{AttnCtx, AttnRegions, DispatchCtx, Frame, Resolver, run};
    use model_compiler::lower::Arg;
    ...
    let (q_pin, o_off) =
        attention_pins(family.as_ref(), lowered, dplan, states_decode_dispatch)?;
    ...
    let crate::facts::FamilyTables { theta_by_layer, rotary_by_layer, softcap, ple_dim, scales } =
        family.tables(&model.checkpoint());
