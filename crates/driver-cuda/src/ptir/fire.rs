...
//!
//! A CALLER that groups. The fire takes one [`Extents`] per lane and writes a
//! record, a descriptor row and a channel-slot row for each; what does not
//! exist is a shell that puts more than one request in a fire, so every fire
//! today has one lane. The remaining question is not layout but policy —
//! whether the lanes of a fire may be different INSTANCES, which would need
//! each lane's channel slots resolved against that instance's own rings
...
impl Prepared {
    ...
    pub fn build(
        ...
        if lane_extents.is_empty() {
            return Err(Error::invalid("ptir::fire", "a fire with no lanes"));
        }
        let lanes = u32::try_from(lane_extents.len())
            .map_err(|_| Error::invalid("ptir::fire", "more lanes than a u32 can count"))?;
        ...
        for (lane, extents) in lane_extents.iter().enumerate() {
            let record = LaneRecord {
                ...
                channel_slot_offset: u32::try_from(lane)
                    ...
                    .ok_or_else(|| {
                        Error::invalid("ptir::fire", "a channel slot offset past u32")
                    })?,
