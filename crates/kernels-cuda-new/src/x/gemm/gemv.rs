...
//! `x::gemm`'s module header — that one is about what
...
//! and it asks `driver_cuda::device::Device::compute_capability` rather than
...
//! not open — the census in [`super`] refuses to grow it for four
...
//! `x::gemm`'s header. **Until they close, this
...
//!
//! # Why this is fired by hand, like `driver_cuda::fire::attn_score`
//!
...
//! `x::gemm`'s header carries the census that refuses
...
//! exactly what `driver_cuda::fire::attn_score` does for its literal `gridDim.y`, and
...
/// `x::gemm`'s `GEMV_ROWS`** — under *The ROW axis*.
...
/// bit-identical are all on `x::gemm`'s
...
///
/// It asked `driver_cuda::device::Device::compute_capability` — the home the
/// DRIVER has for a device attribute — while this program lived in
/// `driver-cuda`. **It cannot, from here.** §5 step 5 moved the host program
/// into `kernels-cuda-new`, and this crate does not depend on the driver
/// shell and must not: `driver-cuda` depends on IT. So the query is spelled
/// the way `runtime::cache`'s `arch()` already spells the same question, in
/// the driver API, against the ordinal the runtime says is bound —
/// `cuDeviceGet` then `cuDeviceGetAttribute(COMPUTE_CAPABILITY_MAJOR)`.
///
/// That is a strictly SMALLER effect than the call it replaces: `Device::bind`
/// forced the primary context where the C++ only read an attribute, and this
/// does not force anything. `cuDeviceGetAttribute` needs no current context,
/// which is the property `runtime::cache` already relies on.
///
...
fn unroll_depth() -> i32 {
    ...
    *DEPTH.get_or_init(|| {
        ...
        // is initialised by the `init` above.
...
/// [`super::GEMV`]'s rows is a refusal here rather than a shifted
