...
pub static KERNELS: &[KernelSig] = &[
    ...
    kernel!(nemotron_mamba_split "ssm::nemotron_mamba_split_bf16",
        // The raw `dt` this cut produces is the join's slot 0 — the
        // `dt_raw` that `nemotron_mamba_ssm` reads as `Source::Aux(0)`.
        // Stated here so the join never has to know this kernel's name.
        publishes_aux = &[(0, 2)],
        operands = operands![
    ...
    kernel!(nemotron_prepare_mamba_params "ssm::nemotron_prepare_mamba_params",
        ...
        // `dt_bias` in the order `Source::Aux(1)`, `Aux(2)` and `Aux(3)`
        // read them.
...
kernel!(nemotron_prepare_mamba_dt_da "ssm::nemotron_prepare_mamba_dt_da",
        // Both a CONSUMER (`dt_bias` below is `Source::Aux(3)`) and a
        // PUBLISHER: its two results are the join's `dt_pre` and `da_pre`,
        // slots 4 and 5, which `nemotron_mamba_ssm` reads. The join collects
        // every publisher before it binds any consumer, so a row being both
        // is not an ordering question.
