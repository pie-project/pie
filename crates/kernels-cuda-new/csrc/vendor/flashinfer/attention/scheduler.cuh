...
// PIE: guarded for NVRTC -- the entire file, 1808 lines of it, and this one is a finding
// rather than a chore: `scheduler.cuh` is the host planner. It reads sequence lengths off
// the CPU, bin-packs requests with a `std::vector` min-heap, carves a workspace with
// `AlignedAllocator` and queries occupancy with
// `cudaOccupancyMaxActiveBlocksPerMultiprocessor` -- and the three `__global__`
// declarations at the top are forward declarations existing only so the occupancy query has
// a function pointer to ask about. There is no device code here to save. Under the JIT the
// plan is Rust's job, computed before a launch and handed to the kernel as parameters, so
// the file is empty to NVRTC by design and not by accident. Guarding it piecewise would
// have taken some thirty markers to reach the same nothing. Upstream is unmodified apart
