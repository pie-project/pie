...
// PIE: guarded for NVRTC -- host-only `<cmath>`, which this file wants for `M_PI` inside
// the Llama-3.1 launcher's smoothing constants -- host arithmetic, guarded with it below.
...
#endif
#include <cstdint>
// PIE: guarded for NVRTC -- host-only. `<iostream>` and `<string>` are
// `PosEncodingModeToString` and the error paths of the launchers.
#ifndef __CUDACC_RTC__
#include <iostream>
#include <string>
#endif
 ...
 */
// PIE: guarded for NVRTC -- returns `std::string`, and is called only from host error
// messages.
#ifndef __CUDACC_RTC__
...
// PIE: guarded for NVRTC -- every launcher in the file -- fourteen `cudaError_t` functions
// from `RopeQuantize` to `BatchQKApplyLlama31Rotary`, each of which computes a grid with
// `std::max`, builds a `void* args[]` and calls `cudaLaunchKernel`. That is precisely what
// this crate replaces: the JIT compiles the `__global__` templates above and Rust fires
// them with `cuLaunchKernel`. 591 lines guarded, and not one of them device code.
#ifndef __CUDACC_RTC__
