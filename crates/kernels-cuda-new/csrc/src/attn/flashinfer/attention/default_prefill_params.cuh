...
// PIE: guarded for NVRTC -- host-only `<cmath>`, and unused besides -- this file is four
// parameter structs and not one of them calls a math function. Upstream is unmodified apart
...
// PIE: guarded for NVRTC -- both `__host__` constructors of `SinglePrefillParams`. NVRTC
// refuses a function explicitly marked `__host__` outright -- "not allowed in JIT mode" --
// so a host constructor is a hard error even where its body would compile. The struct
// itself, and its `__host__ __device__` accessors, are what the kernel reads out of
// `__grid_constant__` and are untouched; Rust fills the fields.
#ifndef __CUDACC_RTC__
...
// PIE: guarded for NVRTC -- both `__host__` constructors of `BatchPrefillRaggedParams`.
// NVRTC refuses a function explicitly marked `__host__` outright -- "not allowed in JIT
// mode" -- so a host constructor is a hard error even where its body would compile. The
// struct itself, and its `__host__ __device__` accessors, are what the kernel reads out of
// `__grid_constant__` and are untouched; Rust fills the fields.
#ifndef __CUDACC_RTC__
...
// PIE: guarded for NVRTC -- both `__host__` constructors of `BatchPrefillPagedParams`.
// NVRTC refuses a function explicitly marked `__host__` outright -- "not allowed in JIT
// mode" -- so a host constructor is a hard error even where its body would compile. The
// struct itself, and its `__host__ __device__` accessors, are what the kernel reads out of
// `__grid_constant__` and are untouched; Rust fills the fields.
