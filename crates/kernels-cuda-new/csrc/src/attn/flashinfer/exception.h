...
// PIE: guarded for NVRTC -- the whole file. `Error` derives from
// `std::exception` and every macro here builds a message in a
// `std::ostringstream`, so nothing in it survives a compiler with no host
// standard library; `FLASHINFER_CHECK` and `FLASHINFER_ERROR` are expanded 49
// times in the closure and every one of those sites is inside host code that
// is itself guarded. Upstream is unmodified apart from markers of this form;
// see NOTICE. Removing them restores the file.
...
#endif  // __CUDACC_RTC__ -- PIE guard, opened above
