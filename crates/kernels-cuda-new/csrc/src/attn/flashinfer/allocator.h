...
// PIE: guarded for NVRTC -- the whole file. `AlignedAllocator` hands out slices of a host-
// side workspace buffer and reports overruns through `FLASHINFER_ERROR`, which is itself
// guarded in `exception.h`; the JIT never allocates. Nothing here is device code. Upstream
// is unmodified apart from markers of this form; see NOTICE. Removing them restores the
// file.
