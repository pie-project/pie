...
// the directive — so the seven angle includes here resolve because
// `csrc/shim/cuda_bf16.h` is carried as `cuda_bf16.h` and
// `csrc/vendor/flashinfer/attention/decode.cuh` as
// `flashinfer/attention/decode.cuh`. The trees are rooted to make that fall
// out; `carried.rs`'s header is the design.
//
// This unit therefore demands `Headers::LibraryAndVendor` (`unit.rs`'s
// `DEMANDS`, keyed `attn/fa2_*`), which is `ALL_HEADERS` — `SHIM + LIBRARY +
// VENDOR`, the 1.7 MB vendored closure included. A unit that forgot to say so
// would get `DEVICE_HEADERS` and fail on line one of `decode.cuh`.
//
