...
/// Whether an emitted step names the op it runs instead of the whole
/// `ptir_step` switch. Native drivers (lavapipe, the desktop Vulkan/Metal
/// backends) recompile the code an entry point reaches for every pipeline,
/// so a stage of hundreds of steps that each reach the whole switch takes
/// minutes; naming the op cuts that to seconds. Chrome's Dawn is the
/// opposite — it reuses the compile of an entry that reaches identical code,
/// so the shared switch is faster there and specialising defeats the reuse.
fn specialize() -> bool {
    !cfg!(target_arch = "wasm32")
}

/// The call `ptir_step(p)` makes for an op of this tag, spelled directly.
/// A pipeline is built per step, and a driver compiles what its entry point
/// reaches: through `ptir_step` that is the runtime's whole switch, every op
/// of it, for every step. Naming the op makes a stage of hundreds of steps
/// build in seconds where lavapipe or Tint took minutes.
...
/// The runtime's reduction kind for a reduce tag, for the same reason.
...
mod tests {
    ...
    fn every_arm_of_the_runtime_switch_is_spelled_directly() {
        ...
        // `step_call` names the op `ptir_step` would dispatch to; the two
        // are read off the same switch, arm by arm.
        let switch = RUNTIME
...
mod stepwise_tests {
    ...
    fn neither_shape_emits_a_call_for_an_elided_node() {
        ...
        let one = emit_launch_stage("whole", &stage).expect("emits");
        let many = emit_launch_steps("s", &stage).expect("emits");
        // Node 1 is the reshape, which reads no bytes: no emitted call takes
        // `1u` as its node argument. (Look past the runtime, whose own code
        // is full of `1u` literals.)
