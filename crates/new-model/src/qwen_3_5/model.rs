...
impl Model {
    pub fn a3b(w1: Repr, tp: u32) -> Model {
        assemble(
            w1,
            tp,
            Dims {
    ...
    pub fn d0_8b(w1: Repr, tp: u32) -> Model {
        assemble(
            w1,
            tp,
            Dims {
    ...
    pub fn d3b(w1: Repr, tp: u32) -> Model {
        assemble(
            w1,
            tp,
            Dims {
...
fn assemble(w1: Repr, tp: u32, d: Dims) -> Model {
        ...
        .map(|l| {
            ...
            let mixer = if attn_at(l) {
                ...
                Mixer::Attn(Attn {
                    ...
                    qg_proj: Weight::sym(n("qg_proj"), [2 * d.q_heads as u64 * hd, hidden], w1)
                        .columns(),
                    k_proj: Weight::sym(n("k_proj"), [d.kv_heads as u64 * hd, hidden], w1)
                        .columns(),
                    v_proj: Weight::sym(n("v_proj"), [d.kv_heads as u64 * hd, hidden], w1)
                        .columns(),
                    o_proj: Weight::sym(n("o_proj"), [hidden, d.q_heads as u64 * hd], w1).rows(),
                ...
                Mixer::Gdn(Gdn {
                    ...
                    in_qkvz: Weight::sym(n("in_qkvz"), [qkvz, hidden], w1)
                        .packed([k_w, k_w, v_w, v_w]),
                    in_ba: Weight::sym(n("in_ba"), [2 * d.v_heads as u64, hidden], w1)
                        .packed([d.v_heads as u64, d.v_heads as u64]),
                    conv: Weight::sym(n("conv"), [qkv, d.conv_kernel as u64], w1)
                        .packed([k_w, k_w, v_w]),
                    dt_bias: Weight::sym(n("dt_bias"), [d.v_heads as u64], w1).columns(),
                    a_log: Weight::sym(n("a_log"), [d.v_heads as u64], Repr::F32).columns(),
                    norm: Norm {
                        weight: Weight::sym(n("gdn_norm"), [d.v_dim as u64], Repr::F32),
                        eps: d.norm_eps,
                    },
                    out_proj: Weight::sym(
                        n("out_proj"),
            ...
            let mlp = match &d.mlp {
                MlpDims::Dense { inter } => Mlp::Dense {
                    gate_up: Weight::sym(n("gate_up"), [2 * *inter as u64, hidden], w1)
                        .packed([*inter as u64, *inter as u64]),
                    down: Weight::sym(n("down"), [hidden, *inter as u64], w1).rows(),
                    inter: *inter,
                },
                MlpDims::Routed(m) => Mlp::Routed {
                    ...
                    shared_down: Weight::sym(n("shared_down"), [hidden, m.shared_inter as u64], w1)
                        .rows(),
                    shared_gate: Weight::sym(n("shared_gate"), [1, hidden], w1),
                    experts: m.experts,
                    top_k: m.top_k,
                    inter: m.inter,
    ...
    Model {
        ...
        head: if d.tied {
            ...
            Head::Bank(Weight::sym("lm_head", [d.vocab as u64, hidden], w1))
        },
        layers,
        final_norm: Norm {
