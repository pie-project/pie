...
pub struct Layer {
    ...
    pub pre_ffw_norm: Norm,
    pub post_ffw_norm: Norm,
...
pub enum AttnBanks {
    Owned { qkv: Weight, k_norm: Norm },
    Shared { q_proj: Weight },
}
...
impl Model {
    pub fn e4b(w1: Repr, tp: u32) -> Model {
        assemble(
            w1,
            tp,
            Dims {
    ...
    pub fn b31(w1: Repr, tp: u32) -> Model {
        assemble(
            w1,
            tp,
            Dims {
...
fn assemble(w1: Repr, tp: u32, d: Dims) -> Model {
            ...
            Layer {
                attn: Attn {
                    ...
                    banks: if shared_at(l) {
                        ...
                        AttnBanks::Owned {
                            qkv: Weight::sym(n("qkv"), [q_w + 2 * kv_w, hidden], w1)
    ...
    Model {
        ...
        ple: (d.ple_dim > 0).then(|| {
            ...
            Ple {
                dim: d.ple_dim,
                model_proj: Weight::sym("ple.model_proj", [d.layers as u64 * ple, hidden], w1),
                model_norm: Norm {
                    weight: Weight::sym("ple.model_norm", [ple], w1),
                    eps: d.norm_eps,
                },
                per_layer: (0..d.layers)
                    .map(|l| PleLayer {
                        ...
                        gate: Weight::sym(format!("layer.{l}.ple_gate"), [ple, hidden], w1),
                        proj: Weight::sym(format!("layer.{l}.ple_proj"), [hidden, ple], w1),
                        norm: Norm {
                            weight: Weight::sym(format!("layer.{l}.ple_norm"), [hidden], w1),
                            eps: d.norm_eps,
                        },
                        scalar: Weight::sym(format!("layer.{l}.ple_scalar"), [1], w1),
                    })
                    .collect(),
            }
        }),
