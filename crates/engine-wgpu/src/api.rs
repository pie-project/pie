...
pub struct Wgpu {
    ...
    /// A device the host opened itself, taken by the first `load`.
...
impl Wgpu {
    ...
    /// An engine on a device the host already opened (see `device::request`).
    #[must_use]
    pub fn with_device(
        boot: DeviceBoot,
        contract_for: ContractFor,
...
/// Native settles each frame inline, so one arm is enough; the browser keeps
/// as many airborne as the runtime asks, since it only ever hears back by
/// callback.
/// Every frame settles through the sink, so the runtime's frames in flight
/// are the shell's.
...
impl Engine for Wgpu {
    ...
    fn load(&mut self, request: LoadRequest) -> EngineResult<Loaded> {
        ...
        let caps = Capabilities {
            ...
            // The envelope's tokens, fold length and attention mask are
            // resolved from the instance's rings at the fire (`Plane::envelope_*`).
            ...
            kv_handle: None,
            media_encode: false,
            // Host-role channels are published as host mirrors of the rings
            // the passes take from and put to (`Plane::register_channel`), so
            // the runtime's channel tickets are honoured: checked against the
            // rings at the fire (`Plane::disagreeing_ticket`), bumped by the
            // pass itself when it lands.
    ...
    /// The shell only ever hears back by callback, so every frame settles
    /// through the sink, whatever the runahead.
...
impl Wgpu {
    /// A step's guests, taken off the lane with the shell's device and the
    /// lanes' readout policies, to run from the step's landing; `None` when
    /// the step attaches nothing. A next fire that reads an instance's
    /// envelope parks until that instance's guests ran (`Plane::envelope_*`).
    #[cfg(feature = "wgpu")]
    fn deferred_guests(
        &mut self,
        submission: &Step,
    ...
    fn fire_step(
        ...
        // An instance serving several lanes of this step (beams, candidates)
        // is attached once, at its first lane, and holds every lane's rows end
        // to end in its rings; the lanes that follow take the spans that follow.
        ...
        // The gates entered here are left by the landing worker once each
        // instance's pass ran, or when the guests drop unrun (a refused fire).
