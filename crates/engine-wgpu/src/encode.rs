...
/// A moment, for the host-side timings (`device::host::Instant`).
#[cfg(feature = "wgpu")]
#[derive(Clone, Copy)]
            ...
            match *arg {
                ArgValue::Buffer(handle) | ArgValue::BufferMut(handle) => {
                    ...
                    if !at.is_multiple_of(4) {
                        ...
                        // A sub-word cut is staged through the host, which reads the
                        // words back first (a browser parks its lane for that read).
                        {
