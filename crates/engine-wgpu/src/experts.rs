...
/// A browser maps no file; `Layout::plan` refuses a streaming layout before
/// one is ever asked for, so this is never built.
...
/// The browser has one thread, so the rows are copied in turn.
#[cfg(target_arch = "wasm32")]
...
/// Native streams a host tier from the mapped artifact between steps.
#[cfg(not(target_arch = "wasm32"))]
fn admit_streaming(_tiers: Tiers) -> Result<()> {
    Ok(())
}

/// The browser has no mapped artifact and no synchronous read of the routes,
/// so every plane must stay resident on the device.
#[cfg(target_arch = "wasm32")]
