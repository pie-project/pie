...
For infrastructures that already use vLLM or SGLang, Pie has experimental support for running them as *drivers* (backend engines), so Pie piggybacks on their kernel implementations. This can give better performance in some environments than the default driver, at the cost of a few missing features (e.g., custom attention masks). See [Drivers](../reference/drivers) for more information.
