...
`[model.driver].type` is a discriminator. Embedded drivers (`portable`, `cuda_native`, `dummy`) run inside the `pie` process. Driver-specific knobs live in `[model.driver.options]`; see the per-driver pages ([CUDA](./drivers/cuda), [Portable](./drivers/portable)) for what each backend accepts.
...
| Key | Default | Description |
|-----|---------|-------------|
| `type` | required | Driver name. One of `portable`, `cuda_native`, `dummy`. See the per-driver reference pages. |
...
Driver-specific knobs in the driver's own vocabulary. Embedded drivers are validated by `pie` before boot. See the per-driver pages ([CUDA](./drivers/cuda), [Portable](./drivers/portable)) for each driver's option set.
...
- Per-driver options: [CUDA](./drivers/cuda), [Portable](./drivers/portable).
