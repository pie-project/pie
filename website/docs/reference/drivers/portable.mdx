...
From source, `cargo install --path worker` builds `pie` with `driver-portable` enabled by default. On Linux, prefer the native `cuda_native` driver (`cuda*` artifacts) for GPU; the portable+CUDA build remains the CUDA path on Windows (`portable-cuda`).
