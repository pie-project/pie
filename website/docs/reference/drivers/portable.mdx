...
From source, `cargo install --path server` builds `pie` with `driver-portable` enabled by default. CUDA-capable portable release artifacts are named `portable-cuda12.8` and `portable-cuda13.0`.
