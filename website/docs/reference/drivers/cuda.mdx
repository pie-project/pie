...
| `memory_profile` | `"balanced"` | Planner profile: `"latency"`, `"balanced"`, `"throughput"`, or `"capacity"`. |
...
`cuda_native` accepts only planner inputs for GPU memory sizing. The driver reports derived forward limits, KV pages, arena size, and Qwen3.5 state slots at startup.
...
| `max_forward_tokens` | `10240` | Maximum tokens summed across all sequences in a batch. |
| `max_forward_requests` | `512` | Maximum sequences per batch. |
