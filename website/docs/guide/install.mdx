...
The Unix installer downloads a release tarball, installs `pie` into `~/.local/bin` by default, and auto-picks a flavor:
...
Linux CUDA builds require the matching NVIDIA driver, CUDA toolkit runtime libraries, and NCCL runtime libraries on the host. On Debian/Ubuntu systems configured with NVIDIA's package repositories, install NCCL with:

```bash
sudo apt install libnccl2 libnccl-dev
```

On Windows, the PowerShell installer downloads a zip archive and installs `pie.exe` into `%LOCALAPPDATA%\Pie\bin` by default. Valid Windows `PIE_FLAVOR` values are `portable-vulkan` and `portable-cuda`:
