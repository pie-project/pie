...
|---|---|
| Linux x86_64 | `portable`, `cuda12.8`, `cuda13.0` |
...
The `cuda*` flavors bundle the embedded `cuda_native` driver (the recommended CUDA path on Linux). Windows CUDA uses `portable-cuda` (the portable driver with a CUDA backend), as there is no native Windows driver.
