...
When you build a [forward pass](../forward/overview) directly rather than using `Context`, attach media by anchor position with `pass.input_image(&image, anchor)` and `pass.input_audio(&audio, anchor)`. `Context::append_*` is the high-level wrapper most inferlets should use.
