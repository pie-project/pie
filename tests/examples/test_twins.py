...
`naive-baseline`. The Python and JavaScript ports are pinned byte-for-byte to the Rust encoder
(`crates/eta-dsl/tests/inferlet_goldens.rs`), so the gate here is the end-to-end
...
Run against a live server::
    ...
    uv run python tests/examples/test_twins.py --attach ws://127.0.0.1:8080
"""
...
class EngineNondeterministic(FileNotFoundError):
    ...
    cross-language comparison cannot say anything about the inferlet librarys."""
