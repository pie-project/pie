...
run() { # <label> <page-with-query> [timeout-ms]
  ...
  echo "== $label"
  node tests/web/tools/headless.mjs . "tests/web/$page" --timeout "$timeout" 2>&1 \
    | grep -v '^\s*at ' \
...
node tests/web/tools/headless.mjs . crates/kernels-wgpu/tools/wgsl-check.html --timeout 300000 | tail -2

echo "== build the host"
./sdk/server/web/build.sh release > /dev/null
...
echo "== page lifecycle"
node tests/web/tools/lifecycle.mjs 2>&1 | grep -E 'ok  |FAIL|lifecycle\]' | cut -c1-140
