...
out=${PIE_MATRIX_OUT:-scripts/bench/browser/results}
mkdir -p "$out"
./tests/browser/tools/inferlets.sh
echo "== browser"
: > "$out/matrix-browser.raw"
for id in $(python3 -c 'import json; print(" ".join(e["id"] for e in json.load(open("tests/browser/matrix.json"))))'); do
  node tests/browser/tools/headless.mjs . "tests/browser/matrix.html?only=$id&entry_timeout=${PIE_MATRIX_ENTRY_TIMEOUT_MS:-240000}${PIE_MATRIX_QUERY:-}" --timeout 300000 >> "$out/matrix-browser.raw" 2>&1 || true
done
...
for id in $(python3 -c 'import json; print(" ".join(e["id"] for e in json.load(open("tests/browser/matrix.json"))))'); do
  node tests/browser/tools/matrix-native.mjs "$ws" "$id" >> "$out/matrix-native.raw" 2>&1 && status=0 || status=$?
...
entries = {e["id"]: e for e in json.load(open("tests/browser/matrix.json"))}
def sampled(id_):
    e = entries.get(id_, {})
