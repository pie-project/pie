//! Device epilogue ops checked against the host: one fire's logits go through
//! `log_softmax`/`exp`, `cumsum`, `sort_desc`, `top_k` over one row and over a
//! `[2, v]` broadcast flattened to `[2v]`, `reduce_argmax` over two rows, and
//! a `pivot_threshold` nucleus, and every result is recomputed on the host
//! from the published logits. The `[2, v]` cases are what a beam or a set of
//! parallel candidates hands the epilogue, and they caught the WGSL runtime's
//! broadcast-as-copy and pivot payload bugs.
...
struct Input {
    /// Skip the sorts (a 20-round merge of `2v` elements is minutes on a
    /// software Vulkan such as lavapipe); the nucleus pivot's one sort stays.
    #[serde(default)]
    light: bool,
}
...
fn first_mismatch<T: PartialEq + std::fmt::Debug>(name: &str, got: &[T], want: &[T], report: &mut Vec<String>) {
    if got.len() != want.len() {
    ...
    if bad.is_empty() {
        ...
        let i = bad[0];
        report.push(format!("{name}: {} of {} differ; first at {i}: got {:?} want {:?}", bad.len(), got.len(), got[i], want[i]));
    }
}
...
fn close(name: &str, got: &[f32], want: &[f32], tol: f32, report: &mut Vec<String>) {
    ...
    let bad: Vec<usize> = (0..got.len()).filter(|&i| (got[i] - want[i]).abs() > tol * (1.0 + want[i].abs())).collect();
    if bad.is_empty() {
        report.push(format!("{name}: ok ({} entries)", got.len()));
    } else {
        let i = bad[0];
...
// The DRY sampler's device-side history match and penalty, as
// `tests/inferlets/dry-repetition-penalty` writes them.
#[derive(Clone, Copy)]
struct DryCfg {
    multiplier: f32,
    base: f32,
...
fn dry_penalty(hist: &Tensor, hlen: &Tensor, vocab: u32, cfg: DryCfg) -> (Tensor, Tensor, Tensor) {
    ...
    let next_idx = min_elem(cast(iota(l), dtype::i32) + 1i32, broadcast(l as i32 - 1, [l]));
    ...
    for n in cfg.allowed_length..=cfg.max_ngram {
        ...
        penalty = select(gt(&votes, &vocab_zero), broadcast(charge, [vocab]), &penalty);
    }
    let count = reshape(reduce_sum(cast(&gt(&penalty, &vocab_zero), dtype::f32)), [1]);
...
async fn main(input: Input) -> Result<String> {
    ...
    let bc_idx = Channel::from(vec![0i32, 1, vocab as i32, vocab as i32 + 1, 2 * vocab as i32 - 1]).named("bcast_idx");
    let offs = Channel::from(vec![0.0f32, -0.5]).named("offs");
    const HIST: [i32; 16] = [5, 6, 7, 5, 6, 7, 5, 6, 0, 0, 0, 0, 0, 0, 0, 0];
    const HLEN: u32 = 8;
    let dry = DryCfg { multiplier: 0.8, base: 1.75, allowed_length: 2, max_ngram: 8, capacity: 16 };
    ...
    fwd.epilogue(move || {
        ...
        let cand = broadcast(reshape(offs.take(), [2, 1]), [2, vocab]) + broadcast(reshape(&logprobs, [1, vocab]), [2, vocab]);
        bc_out.put(&reshape(gather(reshape(&cand, [2 * vocab]), bc_idx.take()), [5]));
    ...
    // Host references from the published logits.
    ...
    let mut acc = 0.0f32;
    let ref_cums: Vec<f32> = probs.iter().map(|p| { acc += p; acc }).collect();
    close("cumsum", &cums, &ref_cums, 1e-2, &mut report);
    let ord = desc_order(&probs);
    let lord = desc_order(&logits);
    let cand: Vec<f32> = logprobs.iter().cloned().chain(logprobs.iter().map(|x| x - 0.5)).collect();
    // The broadcast [2, v] rectangle, sampled at both rows' ends: a copy
    // instead of a broadcast reads past the source here.
    let ref_bc: Vec<f32> = [0usize, 1, v, v + 1, 2 * v - 1].iter().map(|&i| cand[i]).collect();
    ...
    // pivot: keep the maximal descending prefix whose exclusive mass < TOP_P.
    let mut excl = 0.0f32;
    ...
    report.push(format!("pivot keep: kept {kept} want {want_kept} prefix_ok={prefix_ok}"));
    // DRY: the history's suffix match, the penalty it votes, and the
    // scatter that appends the pick to the history.
    ...
    let want_arg = (0..v).max_by(|&a, &b| want_pen[a].partial_cmp(&want_pen[b]).unwrap().then(b.cmp(&a))).unwrap_or(0) as i32;
    report.push(format!(
