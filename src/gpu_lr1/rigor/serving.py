...
def under_load(
    ...
    ours = gpugrammar.Compiler(vocabulary).compile_json_schema(instance["schema"])
    ...
    for count in workers:
        ...
        try:
            rows.append(
                {
                    "workers": count,
                    "gpugrammar": _measure(device_batch.fill_mask, repeats).__dict__,
        ...
        latest = rows[-1]
        print(
            f"  {count:3} busy cores: ours {latest['gpugrammar']['p50']:8.1f}us "
            f"p99 {latest['gpugrammar']['p99']:8.1f} | "
...
def capturability(model: str, schema_index: int, batch: int) -> Answer:
    ...
    return Answer(
        ...
        headline=(
            f"decode-step CUDA graph can contain the fill: gpugrammar "
            f"{'yes' if ours_ok else 'no'}, xgrammar "
            f"{'yes' if theirs_ok else 'no'}"
        ),
        detail={
            "gpugrammar": {"captured": ours_ok, "why": ours_why},
    ...
    if "error" not in load:
        ...
        answers.append(
            Answer(
                ...
                f"while ours goes from {idle['gpugrammar']['p50']:.0f} to "
                f"{busy['gpugrammar']['p50']:.0f} us "
                f"({busy['gpugrammar']['p50'] / max(1e-9, idle['gpugrammar']['p50']):.2f}x)",
                detail=load,
            )
        )
    else:
