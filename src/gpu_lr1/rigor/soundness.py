...
def over_acceptance(
    ...
    our_compiler = gpugrammar.Compiler(BYTE_VOCABULARY)

    engines: dict[str, Any] = {"gpugrammar": None}
    bitmask = None
    torch = None
    if with_xgrammar:
    ...
    for index, instance in enumerate(schemas):
        if limit is not None and compiled["gpugrammar"] >= limit:
            break
        schema = json.loads(instance["schema"])

        built: dict[str, Any] = {}
        try:
            built["gpugrammar"] = _ours(instance["schema"], our_compiler)
