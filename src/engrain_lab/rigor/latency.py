            ...
            rows.append(
                {
                    ...
                    "batch": batch,
                    "engrain": {k: asdict(v) for k, v in our.items()},
...
def _answers(rows: list[dict[str, Any]], forward: dict[int, float]) -> list[Answer]:
    ...
    ratios = by_batch(
        lambda row: total(row, "xgrammar") / max(1e-9, total(row, "engrain"))
    )
    overlapped = by_batch(
        lambda row: unoverlapped(row, "xgrammar")
        / max(1e-9, unoverlapped(row, "engrain"))
    )
    ...
    answers = [
        ...
        Answer(
            ...
            "; ".join(
                ...
                f"p50 {row['engrain']['fill']['p50']:.0f} "
                f"p99 {row['engrain']['fill']['p99']:.0f}us, xgr "
    ...
    if forward:
        ...
        for row in rows:
            ...
            if step:
                fraction[row["batch"]] = {
                    ...
                    "engrain_pct": round(100 * total(row, "engrain") / step, 2),
        ...
        answers.append(
            Answer(
                ...
                + ", ".join(
                    f"batch {b}: ours {v['engrain_pct']}% vs xgr {v['xgrammar_pct']}%"
