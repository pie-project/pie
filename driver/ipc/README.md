...
[`pie-schema`](../../protocol/schema) crate (the dependency floor); this
crate is the transport machinery layered over it.

## Layout

- `src/wire.rs` — thin encode/access/deserialize helpers around
  `rkyv::to_bytes` / `rkyv::access` over `pie_schema`'s frame types.
...
- `src/python.rs` — PyO3 wrappers (`ShmemServer` / `Lease`) for Python
  downstream drivers. Schema `Py<T>` classes live in `pie-schema`.
- `include/pie_ipc.h` — C header for the in-proc vtable
  (`PieInProcVTable`) plus the header-only `pie_ipc/inproc_server.hpp`
  loop; both include `pie_schema.h` for the descriptor types.
- `tests/` — shmem round-trip + wire/cabi tests.

## Features

- `ipc` (default) — shmem ring transport.
- `cabi` (default) — forwards to `pie-schema/cabi` so the C-ABI
  descriptor accessors are available to downstream drivers.
- `python` — PyO3 shmem wrappers (forwards to `pie-schema/python`).

## Cross-language story

| Consumer                   | Path                                                        |
|----------------------------|-------------------------------------------------------------|
| Rust (`pie`, dummy driver) | `pie_ipc::{ipc, ffi, wire}` + schema types from `pie_schema`|
| C++ (`driver/cuda`, etc.)  | Include `pie_ipc.h` + `pie_schema.h`; fill the vtable        |
| Python (`driver/vllm`, …)  | `pie_ipc` maturin wheel → `from pie_ipc import ShmemServer`  |

## Handshake

The `SCHEMA_HASH` (built in `pie-schema` from the schema sources,
