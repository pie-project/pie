...
// Templated so it gathers any element type bit-exactly — the mtp_native_verify
// program gathers I32 token ids (`picked`), not just F32 (charlie, Stage-2):
// a Gather inherits its source dtype (codegen `dt(op.a)`), so an I32 source emits
// `int*` buffers that a hardcoded `float*` primitive rejects at NVRTC compile.
template <typename T>
__device__ __forceinline__ void pie_ir_gather(
        const T* __restrict__ src, int src_len, const int* __restrict__ idx,
        T* __restrict__ dst, int n) {
    int g = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = gridDim.x * blockDim.x;
    for (int i = g; i < n; i += stride) {
        int s = idx[i];
        dst[i] = (s >= 0 && s < src_len) ? src[s] : (T)0;
    }
}
