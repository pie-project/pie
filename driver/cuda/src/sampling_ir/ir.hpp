...
// `MtpDrafts` (id 2) is the device-resident spec-decode drafts channel (echo's
// additive `ir::Binding::MtpDrafts`, IntrinsicId 6): unlike Logits/MtpLogits it
// is NOT a `ws.logits` row but a SEPARATE I32 `[k]` device buffer (bravo's
// retained `mtp_drafts`, source-selected by the executor into `ctx.mtp_drafts`).
enum class Intrinsic : std::uint8_t { Logits = 0, MtpLogits = 1, MtpDrafts = 2 };
