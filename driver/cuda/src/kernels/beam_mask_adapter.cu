...
// contiguous in the logical grid `[lane·STRIDE .. lane·STRIDE+klen[lane])`
...
// One block per lane. Each thread packs a strided subset of the lane's klen
// bits. `mask_dense` is [LANES, STRIDE] with one byte per cell (0/1).
// `mask_indptr` is the per-lane BYTE offset into `packed` ([LANES+1],
// prefix-summed on the host from ceil(klen[l]/8)). `packed` is pre-zeroed.
__global__ void pack_dense_mask_kernel(
    const std::uint8_t* __restrict__ kvm_dense,   // [LANES, STRIDE] bytes (0/1)
...
// Pack the dense [LANES, STRIDE] mask into the FlashInfer packed bitmap.
...
// of ceil(klen[l]/8) — built on the host from the same klen the attention call
// uses.
