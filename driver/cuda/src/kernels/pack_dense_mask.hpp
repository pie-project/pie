...
// Pack a program's dense `[LANES, STRIDE]` per-cell mask (one byte/cell, 0/1,
// from an `AttnMask` descriptor port) into FlashInfer's bit-packed custom mask,
// per lane, over its `klen[l]` physical span. `mask_indptr` ([LANES+1], device)
// is the per-lane BYTE-offset CSR (prefix-sum of ceil(klen[l]/8)); `packed` must
// be pre-zeroed and sized to `mask_indptr[LANES]` bytes. Bit layout matches
// `brle::decode`: bit `q·kv_len + j` (q=0 for a single query), `packed[bit/8] >>
// (bit%8) & 1`.
void launch_pack_dense_mask(
    const std::uint8_t* kvm_dense,        // [LANES, STRIDE] bytes (0/1)
