...
// top_k row-local: k iterations of block-argmax with masking. Emits values
// (desc) and their column indices. Correct for small k (beam widths).
__global__ void k_topk_rows(const float* __restrict__ in, float* __restrict__ out_val,
                            std::uint32_t* __restrict__ out_idx,
                            std::uint32_t rows, std::uint32_t len, std::uint32_t k) {
    __shared__ float sh_v[kTier0Block];
    __shared__ std::uint32_t sh_i[kTier0Block];
    extern __shared__ std::uint8_t taken_raw[];   // len bytes: already-picked mask
    const std::uint32_t row = blockIdx.x;
    if (row >= rows) return;
    const float* r = in + (std::uint64_t)row * len;
    std::uint8_t* taken = taken_raw;
    for (std::uint32_t i = threadIdx.x; i < len; i += blockDim.x) taken[i] = 0;
    __syncthreads();
    ...
    for (std::uint32_t pick = 0; pick < k; ++pick) {
        ...
        std::uint32_t bi = 0;
        for (std::uint32_t i = threadIdx.x; i < len; i += blockDim.x) {
            if (!taken[i] && r[i] > best) { best = r[i]; bi = i; }
        }
