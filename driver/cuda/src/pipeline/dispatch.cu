...
bool Dispatch::run(const pie_native::LaunchView& view,
    ...
    for (std::size_t p = 0; p < n_prog; ++p) {
        ...
        for (auto& output : outs) {
            const DeviceHostChannelTicket* ticket =
                find_publish_ticket(tickets, output.slot);
            ...
            published.push_back({
                .slot = output.slot,
                .target = s.channels.schedule_host_publish_at(
                    output.slot,
        ...
        for (const DeviceHostChannelTicket& ticket : tickets) {
            ...
            consumed.push_back({
                ...
                .wait_id = s.channels.writer_wait_id(ticket.slot),
                .words = ticket.words,
            });
        }
        ...
        launch_publish_host_channel_actuals(
            ...
            snapshot.device,
            callback_stream);
        if (device_tickets != nullptr) {
            CUDA_CHECK(cudaFreeAsync(device_tickets, callback_stream));
        }
