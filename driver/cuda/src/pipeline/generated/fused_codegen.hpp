...
inline constexpr std::uint16_t kCudaGeneratedEmitterVersion = 11;
...
inline GeneratedKernelSource emit_fused_region_cuda(
    ...
    const auto& direct_argmax_skipped = direct_argmax.skipped;
...
static_assert(sizeof(PtirLaneRecord) == 72, "lane record ABI");
...
__device__ __forceinline__ void ptir_fast_argmax(
    ...
    m1_u8* output,
    const M1ValueDesc input_desc) {
  __shared__ M1ArgmaxCandidate float_candidates[256];
  __shared__ M1IntArgmaxCandidate int_candidates[256];
  int* result = reinterpret_cast<int*>(output);
  for (m1_u32 row = 0; row < input_desc.rows; ++row) {
    const m1_u32 base = row * input_desc.last;
    if (input_desc.dtype == 0u) {
      ...
      float_candidates[threadIdx.x] = candidate;
      __syncthreads();
      for (m1_u32 offset = blockDim.x / 2u;
           offset > 0u;
           offset >>= 1u) {
        if (threadIdx.x < offset)
          float_candidates[threadIdx.x] =
              m1_argmax_combine(
                  float_candidates[threadIdx.x],
                  float_candidates[threadIdx.x + offset]);
        __syncthreads();
      }
      if (threadIdx.x == 0u)
        result[row] = (int)float_candidates[0].index;
      __syncthreads();
      continue;
    }
    ...
    int_candidates[threadIdx.x] = candidate;
    __syncthreads();
    for (m1_u32 offset = blockDim.x / 2u;
         offset > 0u;
         offset >>= 1u) {
      if (threadIdx.x < offset)
        int_candidates[threadIdx.x] =
            m1_int_argmax_combine(
                int_candidates[threadIdx.x],
                int_candidates[threadIdx.x + offset]);
      __syncthreads();
    }
    if (threadIdx.x == 0u)
      result[row] = (int)int_candidates[0].index;
    __syncthreads();
  }
}
...
__device__ __forceinline__ void ptir_fast_argmax_intrinsic(
    ...
    m1_u32 stride,
    m1_u32 row_offset) {
  __shared__ M1ArgmaxCandidate candidates[256];
  int* result = reinterpret_cast<int*>(output);
  for (m1_u32 row = 0; row < input_desc.rows; ++row) {
    M1ArgmaxCandidate candidate{
        m1_neg_inf(), 0u, 0u, 0u};
    ...
    if (threadIdx.x == 0u)
      result[row] = (int)candidates[0].index;
    __syncthreads();
  }
}
