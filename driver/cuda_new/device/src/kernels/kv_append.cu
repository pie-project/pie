...
// Lifted verbatim from driver/cuda/src/kernels/kv_paged.cu (the RAW-POINTER
// bf16 variant). The kernel body (`write_kv_kernel<HND_LAYOUT>`), the
// `find_request` device helper, and the launcher (renamed from
// `launch_write_kv_to_pages_bf16` to `write_kv_to_pages_bf16`) are unchanged
