#include "mla_write.cuh"

#include <cuda_bf16.h>

// Lifted verbatim from driver/cuda/src/kernels/mla_paged.cu (the RAW-POINTER
// bf16 variant). The kernel body (`write_mla_kernel`), the `find_request` and
// `resolve_dst` device helpers, and the launcher (renamed from
// `launch_write_mla_to_pages_bf16` to `write_mla_to_pages_bf16`) are unchanged
// apart from the namespace (`pie_cuda_driver::kernels` ->
// `pie_cuda_device::kernels`) and the dropped `launch_` prefix.
//
// Dropped: the `launch_write_mla_to_pages` overload (it needs MlaCacheLayerView
// from mla_cache.hpp) and the post-launch `CUDA_CHECK(cudaGetLastError())` (the
// ABI layer centralizes that, mirroring kernels/kv_append.cu and
// ops/mla_paged.cu), so this TU never includes mla_cache.hpp / cuda_check.hpp.
