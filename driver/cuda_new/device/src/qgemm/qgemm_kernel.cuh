#pragma once
//
// Fused quant GEMM kernel template (qgemm).
//
// Internalized and de-branded from vLLM's int4 weight-only GEMM kernel
// (originally adapted from the IST-DASLab fused dequant + tensor-core MMA
// pipeline, Apache-2.0). The torch / external ScalarType coupling has been
// replaced by the minimal compile-time `QType` tag (qtype.h); the dtype map,
// dequant routines and MMA wrappers live in the sibling qgemm_*.cuh headers.
//
// The kernel fuses async global->shared staging of activations + packed
// quantized weights, in-register dequantization (with the quantization scale
// folded in), and m16n8k16 tensor-core MMA with fp32 accumulation, then a
// striped cross-threadblock reduction. This slice instantiates only the
// bf16-activation x u4b8-weight -> bf16-output path; the body stays generic so
// fp8/mxfp4/int8 weight paths can be added as new dequant specializations.

#include "qgemm_common.cuh"
#include "qgemm_dtypes.cuh"
#include "qgemm_dequant.cuh"
#include "qgemm_mma.h"
#include "qtype.h"
...
template <typename scalar_t,  // compute dtype, half or nv_float16
          const QTypeId b_type_id,  // weight QTypeMap id
          const QTypeId s_type_id,  // weight scale ScalarType id
...
template <int count, QTypeId type_id>
__device__ inline void ldsm(typename QTypeMap<type_id>::FragA& frag_a,
                            const void* smem_ptr) {
...
template <QTypeId type_id>
__device__ inline void scale(typename QTypeMap<type_id>::FragB& frag_b,
                             typename QTypeMap<type_id>::FragS& frag_s,
                             int i) {
  using scalar_t = typename QTypeMap<type_id>::scalar_t;
  using scalar_t2 = typename QTypeMap<type_id>::scalar_t2;
  scalar_t2 s = QTypeMap<type_id>::num2num2(
...
__device__ inline void scale_and_sub(
    typename QTypeMap<type_id>::FragB& frag_b,
    typename QTypeMap<type_id>::scalar_t s,
    typename QTypeMap<type_id>::scalar_t zp) {
  using scalar_t = typename QTypeMap<type_id>::scalar_t;
  using scalar_t2 = typename QTypeMap<type_id>::scalar_t2;
  scalar_t2 s2 = QTypeMap<type_id>::num2num2(s);
  scalar_t2 zp2 = QTypeMap<type_id>::num2num2(zp);
...
__device__ inline void sub_zp(
    typename QTypeMap<type_id>::FragB& frag_b,
    typename QTypeMap<type_id>::scalar_t2& frag_zp, int i) {
  using scalar_t = typename QTypeMap<type_id>::scalar_t;
  using scalar_t2 = typename QTypeMap<type_id>::scalar_t2;
  scalar_t2 zp = QTypeMap<type_id>::num2num2(
...
__device__ inline void scale4(
    typename QTypeMap<type_id>::FragB& frag_b,
    typename QTypeMap<type_id>::FragS& frag_s_1,
    typename QTypeMap<type_id>::FragS& frag_s_2,
    typename QTypeMap<type_id>::FragS& frag_s_3,
    typename QTypeMap<type_id>::FragS& frag_s_4, int i) {
  using scalar_t = typename QTypeMap<type_id>::scalar_t;
  using scalar_t2 = typename QTypeMap<type_id>::scalar_t2;
...
__device__ inline void scale_float(
    float* c, typename QTypeMap<type_id>::FragS& s) {
  using scalar_t = typename QTypeMap<type_id>::scalar_t;
  scalar_t* s_ptr = reinterpret_cast<scalar_t*>(&s);
  c[0] = __fmul_rn(c[0], QTypeMap<type_id>::num2float(s_ptr[0]));
  c[1] = __fmul_rn(c[1], QTypeMap<type_id>::num2float(s_ptr[1]));
}
...
template <const QTypeId a_type_id,  // A ScalarType id
          const QTypeId b_type_id,  // B ScalarType id
          const QTypeId c_type_id,  // C ScalarType id
          const QTypeId s_type_id,  // B_SCALE ScalarType id
          const int threads,          // number of threads in a threadblock
          const int thread_m_blocks,  // number of 16x16 blocks in the m
                                      // dimension (batchsize) of the
                                      // threadblock
...
__global__ void QGemm(
  ...
  if constexpr (a_type_id == kFE4M3fn.id()) return;
  #endif

  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 750
  // Turing TensorCore only supports fp16 and int8
  if constexpr (a_type_id != kFloat16.id() && a_type_id != kS8.id())
    return;
  #endif

  #if defined(__CUDA_ARCH__) && __CUDA_ARCH__ == 750
  constexpr auto num_bits = QType::from_id(b_type_id).size_bits();
  // Disable use_fp16_accum for NVFP4 and cases when group_size == -1 &&
  // num_bits == 4
  constexpr bool use_fp16_accum =
      a_type_id == kFloat16.id() &&
      (!(b_type_id == kFE2M1f.id() && s_type_id == kFE4M3fn.id()) &&
       !(group_blocks == -1 && num_bits == 4));
  #else
  constexpr bool use_fp16_accum = false;
  #endif
  using Adtype = QTypeMap<a_type_id>;
  using Cdtype = QTypeMap<c_type_id>;
  const int4* A = A0;
  int4* C = C0;

  using scalar_t = typename QTypeMap<a_type_id>::scalar_t;
  using scalar_t2 = typename QTypeMap<a_type_id>::scalar_t2;
  using scalar_32bit_t = typename QTypeMap<a_type_id>::scalar_32bit_t;

  using c_scalar_t = typename QTypeMap<c_type_id>::scalar_t;
  using c_scalar_t2 = typename QTypeMap<c_type_id>::scalar_t2;

  using FragA = typename QTypeMap<a_type_id>::FragA;
  using FragB = typename QTypeMap<a_type_id>::FragB;
  using FragC = typename QTypeMap<a_type_id>::FragC;
  using FragS = typename QTypeMap<c_type_id>::FragS;
  using FragZP = typename QTypeMap<c_type_id>::FragZP;

  static constexpr auto a_type = QType::from_id(a_type_id);
  static constexpr auto b_type = QType::from_id(b_type_id);
  static constexpr auto c_type = QType::from_id(c_type_id);
  static constexpr auto s_type = QType::from_id(s_type_id);
  if constexpr (b_type == kFE2M1f) {
    static_assert(s_type == kFE4M3fn && group_blocks == 1 ||
                  s_type == kFE8M0fnu && group_blocks == 2);
  } else if constexpr (s_type == kFE8M0fnu) {
    // MXFP8: FP8 weights with e8m0 microscaling block scales
    static_assert(b_type == kFE4M3fn && group_blocks == 2);
  } else if constexpr (std::is_same<scalar_t, nv_bfloat16>::value) {
    static_assert(s_type == kBFloat16);
  } else if constexpr (std::is_same<scalar_t, half>::value) {
    static_assert(s_type == kFloat16);
  }
  ...
  constexpr bool has_zp = b_type == kU4 || b_type == kU8;
  constexpr bool is_int_type = b_type == kU4 || b_type == kU8 ||
                               b_type == kS4 || b_type == kS8 ||
                               b_type == kU4B8 || b_type == kU8B128;
  // see comments of dequant.h for more details
  constexpr bool dequant_skip_flop =
      is_a_8bit || (b_type == kFE4M3fn && !(s_type == kFE8M0fnu)) ||
      b_type == kFE2M1f && s_type == kFE4M3fn ||
      has_zp && !is_zp_float && !std::is_same<scalar_t, nv_bfloat16>::value ||
      has_zp && !is_zp_float && !(b_type == kU8);

  float global_scale_f32 = 1.0f;

  if constexpr (b_type == kFE2M1f && s_type == kFE4M3fn) {
  ...
  auto matmul = [&](int k, int pipe) {
    ...
    if constexpr (s_type == kFE4M3fn || s_type == kFE8M0fnu) {
  ...
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      if constexpr (b_type_id == kFE2M1f.id()) {
  ...
  #pragma unroll
            ...
            float2 s_vals[2];
            if constexpr (s_type_id != kFE8M0fnu.id()) {
