#pragma once
//
// Common device helpers + tile constants for the fused quant GEMM.
// (De-branded port of the upstream `marlin.cuh` building blocks: cp.async
//  wrappers, the Vec<> fragment container, and the tile-size constants.)
...
// 8 warps: every SM has 4 schedulers, >1 warp per scheduler hides latency,
// while keeping few enough warps for many registers / small tiles.
