...
// MetalExecutor — the forward seam between entry.cpp's PTIR launch path and
// the raw_metal decode pipeline (metal_ptir_plan.md §5.1, D1: RawMetalDecoder
// backs it, not the MLX ops path). Deliberately narrow: `entry.cpp` never
// includes RawMetalDecoder or any Metal/ObjC header directly — it owns one
// `std::unique_ptr<MetalExecutor>`, created lazily on the first
// forward-needing launch, and calls `setup()` / `forward()` through this
// plain-C++ interface only. The real implementation (executor.cpp) is
...
//
// Sealed M=1 scope (ordinary linear member): RawMetalDecoder holds
...
struct MemberForwardDesc {
    // The engine's stable identity for this PTIR instance (entry.cpp passes
    ...
    // this build, see executor.cpp) rather than ignoring it and running the
...
struct SetupConfig {
    ...
    // allocates a REAL paged KV pool sized from these (see RawMetalDecoder::
...
//       `close_sequence()` the old one first (entry.cpp's
...
class MetalExecutor {
  public:
    ...
    // One-time lifecycle: load the qwen3.6 checkpoint via RawMetalDecoder.
    ...
    // instance (`MetalDriver::close_instance`) so a later FRESH session is
    ...
    // control ops — narrow methods so entry.cpp never needs to include
    // RawMetalDecoder/Metal types directly.
