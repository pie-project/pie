...
use engrain_tables::{Artifact, SetKind};
...
fn max_configs() -> usize {
    ...
    *LIMIT.get_or_init(|| {
        std::env::var("ENGRAIN_MAX_CONFIGS")
            .ok()
...
impl Matcher {
    ...
    fn successors(&self, group: &engrain_tables::GroupEntry) -> Vec<Config> {
    ...
    for terminal in terminals {
        ...
        while let Some(stack) = agenda.pop() {
            ...
            for value in actions {
                if value == engrain_lr::tables::ACCEPT {
                    ...
                    if settled.len() < MAX_PATHS {
                        ...
                        shifted.push(engrain_lr::tables::decode_shift(value) as u32);
...
fn reduce_once(artifact: &Artifact, mut stack: Vec<u32>, value: i32) -> Option<Vec<u32>> {
    let production = engrain_lr::tables::decode_reduce(value);
