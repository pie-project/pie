...
use engrain_tables::pipeline::{Failure, Limits, compile_json_schema};
...
fn main() -> Result<()> {
    ...
    if arguments.len() < 2 {
        bail!("usage: engrain-validate <instances.json> [max-reported]");
    }
    ...
    let limits = Limits {
        ...
        exact: std::env::var_os("ENGRAIN_EXACT").is_some(),
        ..Default::default()
    };
    let report = std::env::var("ENGRAIN_REPORT").is_ok();
