...
use engrain_lex::lexicon::{extract, terminal_automata_within};
...
use engrain_run::cache::Cache;
use engrain_tables::{emit, emit_ungrouped};
...
fn main() -> Result<()> {
    ...
    if arguments.len() < 2 {
        bail!("usage: engrain-cache <instances.json> [limit]");
    }
    ...
    let limit: usize = arguments
        ...
        .unwrap_or(24);
    let state_limit: usize = std::env::var("ENGRAIN_MAX_LEXER_STATES")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(4_000);
    ...
    for (index, instance) in corpus.instances.iter().enumerate() {
        ...
        let cfg = match flatten_within(&lexicon, engrain_lr::cfg::DEFAULT_PRODUCTION_BUDGET) {
            Some(cfg) => cfg,
            None => continue,
        };
