...
extern "C" __global__ void en_locate(
    ...
    int32_t vocabulary) {
    en::Shape shape{batch, configs, stack_stride, 0, rows};
    ...
    for (int32_t slot = warp; slot < total; slot += warps) {
        int32_t row = en::owner(live_offsets, rows, slot);
        ...
        int32_t best =
            en::locate_one(arena, state, shape, token, sequence, row, lane, has_verdicts != 0,
                           vocabulary);
        if (lane == 0) {
            found[row] = best;
        }
