...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=ENGRAIN_SKIP_CUDA");
    ...
    // and it includes the rest. See the note at the top of `engrain.cu`.
    let unit = kernels.join("engrain.cu");
    assert!(unit.exists(), "kernels/engrain.cu is the translation unit");

    let out = PathBuf::from(std::env::var("OUT_DIR").expect("cargo sets OUT_DIR"));
    let fatbin = out.join("engrain.fatbin");
    ...
    if std::env::var_os("ENGRAIN_SKIP_CUDA").is_some() {
