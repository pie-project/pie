...
extern "C" __global__ void gg_mask(
    ...
    for (int32_t item = thread; item < total; item += threads) {
        int32_t row = gg::owner(work_offsets, rows, item);
        ...
        if (has_verdicts) {
            int32_t stride =
                arena->verdict_stride[gg::base_of(arena, grammar, gg::B_VERDICT_STRIDE) + lexer];
            if (stride > 0) {
        ...
        if (settled == 0) {
            ...
            gg::Verdict v = gg::replay_group(t, state->stack, base, mine, probe_window,
