...
extern "C" __global__ void gg_probe_identity(int32_t* out, int32_t count, int32_t bias) {
...
extern "C" __global__ void gg_probe_accumulate(int32_t* out, int32_t count, int32_t by) {
    int32_t index = blockIdx.x * blockDim.x + threadIdx.x;
    if (index < count) {
        out[index] += by;
    }
