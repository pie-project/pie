// Turn the `RESULT {...}` lines of results/*.log into one markdown table.
// `node web/bench/compare.mjs [results-dir]`
...
for (const file of readdirSync(dir).filter((f) => f.endsWith(".log")).sort()) {
  ...
  if (!r.bench) continue; // corner-only logs have no numbers to tabulate
