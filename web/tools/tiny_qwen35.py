...
"""Writes a random Qwen3.5 snapshot the `qwen35-tiny` row imports.
    ...
    python3 web/tools/tiny_qwen35.py <out-dir> [--tokenizer <dir-with-tokenizer.json>]
...
def bf16(values):
    # Truncating f32 -> bf16 (round-to-nearest is not needed for random weights).
...
def build(rng):
    ...
    out = []
    fan = 1.0 / (HIDDEN ** 0.5)
    out.append((p + "embed_tokens.weight", [VOCAB, HIDDEN], "BF16", None))  # filled lazily: big
...
def write_safetensors(path, tensors, rng):
    ...
    with open(path, "wb") as f:
        ...
        for (name, shape, dtype, data), size in zip(tensors, sizes):
            if data is None:
                # The embedding: written in rows so the script stays small in memory.
                scale = 0.02
