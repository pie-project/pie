// One benchmark for both hosts. Takes a connected `PieClient` (the JavaScript
// SDK) and an installed program name, so the same code measures the native
// server over a WebSocket and the tab over the in-page transport.
//
// `text-completion` returns everything at once, so per-token latency is
// derived: decode = (t(N tokens) − t(1 token)) / (N − 1), prefill ≈ t(1 token).

const SHORT = "The capital of France is";

/** A prompt of roughly `words` words with a predictable token count. */
...
/**
 * Latency and throughput scenarios. Returns a plain object of numbers.
 * `opts.carried` names a device-carried inferlet (naive-baseline) to time
 * beside the host-driven one; `opts.memory()` reports the host's memory in
 * MiB (possibly as a promise), for the leak check; `opts.fixture` says the
 * model is a random-weight fixture, whose words mean nothing.
 */
export async function runBench(client, program, log = () => {}, opts = {}) {
  const r = {};

  // Warm once: pipelines compile on the first frames.
  const warm = await complete(client, program, { prompt: SHORT, max_tokens: 4 });
  r.warm_ms = +warm.ms.toFixed(1);
  log(`warm: ${r.warm_ms} ms`);

  // Time to first token: prefill of a short prompt + one decode.
  const ttft = [];
  ...
  // Decode: per-token cost from two lengths.
  const t64 = [];
  ...
  // Prefill of long prompts (chunked past max_forward_tokens).
  ...
  // Concurrency: N processes at once, 32 tokens each.
  ...
  // Stability over many short runs.
  const many = [];
  ...
  // A device-carried decode loop: the epilogue samples on the device and the
  // next fire embeds that token, so each step is a fire that depends on the
  // previous one (in the tab, a park per step).
  if (opts.carried) {
  ...
  // Leak check: many short runs, memory before and after.
  if (opts.memory) {
...
/** Behaviour at the edges. Returns [{name, ok, detail}]. */
export async function runCorners(client, program, log = () => {}, opts = {}) {
  ...
  if (opts.makeClient) {
    await check("two clients at once", async () => {
      ...
      // A fixture model (random weights) answers with something, not with
      // the capitals: the two prompts must each get a completion of their own.
      if (opts.fixture) {
        const [a, b] = d.split(" | ");
