//! Shared implementation for benchmark-focused reasoning inferlets.
//!
...
impl Input {
    pub fn force_pattern(&mut self, pattern: &'static str) {
...
pub async fn run(input: Input) -> Result<Output> {
    validate(&input)?;
