...
use crate::serve::coordination::{self, CoordinationArgs, Coordinator};
...
pub struct ServeArgs {
    ...
    /// Cluster-coordination topology: `--single-node` (embed the controller
    /// in-proc; the default) or `--role=<…> --controller=<addr>` (join a
    /// distributed cluster).
    #[command(flatten)]
    pub coordination: CoordinationArgs,
}
...
pub fn run(args: ServeArgs) -> Result<()> {
    ...
    if args.monitor {
        run_with_monitor(cfg, coordinator)
    } else {
        serve::run_with_config(cfg, coordinator)
    }
}

/// Boot the engine + run the TUI in the same process. Mirrors
/// `pie/src/pie_cli/commands/serve.py`'s `--monitor` branch.
fn run_with_monitor(cfg: config::Config, coordinator: Coordinator) -> Result<()> {
    ...
    let engine = runtime
        .block_on(async { serve::start_engine(cfg, coordinator).await })
