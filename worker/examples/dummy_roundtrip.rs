...
//! against the live `pie-worker` + `driver-dummy` stack.
//!
//! Run with:
//!
//! ```bash
//! cargo build -p pie-worker --no-default-features --release
...
//! cargo run --example dummy_roundtrip -p pie-worker \
...
async fn main() -> Result<()> {
    ...
    // The child writes:
    //   pie-worker serving on 127.0.0.1:8093 (...)
    ...
    let _reader = thread::spawn(move || {
        ...
        while reader.read_line(&mut line).map_or(false, |n| n > 0) {
            print!("[pie] {line}");
            if line.contains("pie-worker serving on") {
                serving = true;
...
fn repo_root() -> Result<PathBuf> {
    // CARGO_MANIFEST_DIR is `<repo>/worker`; go up two.
