"""
Mock WIT bindings for unit-testing the inferlet library outside a componentize-py
build.
...
SCOPE: the non-forward interfaces only, which is the whole of the inferlet library's
hand-written layer today. The forward-pass surface has no Python counterpart
yet -- see `inferlet/__init__.py` and `scripts/check-inferlet-interfaces.sh`. When
...
right binding and marshals the result, and no more. That the inferlet library matches the
REAL world is proven by actually building a component --

    componentize-py -d crates/inferlet/wit -w inferlet componentize \\
        -p . -p python/inferlet/src -p python/inferlet/src/inferlet/bindings app
...
class _ScriptedDecoder:
    ...
    Tests set `script` before constructing the inferlet library's wrapper, because the
