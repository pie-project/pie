//! The model service: the runtime's global model/tokenizer cache.
//!
//! Split out of `lib.rs` so the whole half gates with the `chat` aspect it
//! serves; every item is re-exported at the crate root, so consumers keep
//! their paths.
...
use crate::instruct::{self, Instruct};
