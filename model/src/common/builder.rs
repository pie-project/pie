...
//! [`policy`](crate::common::policy) argues for.
...
impl<'a> Builder<'a> {
    ...
    /// else.
    ///
    /// [`Projections::InPlace`]: crate::common::policy::Projections::InPlace
    pub fn dense_fused_projection_joins(&mut self) -> Result<(), Error> {
        use crate::common::policy::Projections;
...
/// The contract for a family that needs nothing beyond the name-pattern
/// rules.
///
...
/// them one by one. The MoE slice runs first because a family can have both a
/// fused expert weight and fused dense projections (Gemma-4 does).
pub fn author_dense_contract(b: &mut Builder<'_>) -> Result<(), Error> {
