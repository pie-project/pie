...
struct Input {
    ...
    /// derived: the plane count is not in the model profile and the inferlet library has no
...
async fn main(input: Input) -> Result<Output> {
    ...
    // The split is `prefill_chunks` (the inferlet library), which spreads the remainder over the
