...
//! feels it. Through `scripts/bench/pie_bench.py latency` (4 requests x 256
...
//! `scripts/bench/pie_bench.py latency --requests 4 --max-tokens 256`, that is
...
//! `scripts/bench/pie_bench.py` passes to every launched inferlet since
...
struct Input {
    ...
    /// (`scripts/bench/pie_bench.py` passes its `--concurrency`). Absent, one. Read
...
struct Output {
    ...
    tokens: Vec<u32>,
    count: usize,
    /// `text-completion-bench`'s envelope, for `scripts/bench/pie_bench.py`.
...
/// token a fire. Measured through `scripts/bench/pie_bench.py`, a list of European
