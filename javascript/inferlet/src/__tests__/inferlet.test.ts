// Unit tests for the inferlet library's hand-written layer, against the stubs in
...
describe('package surface', () => {
  ...
  it('does not re-export the removed forward surface', () => {
    ...
    // scripts/check-inferlet-interfaces.sh green while the inferlet library still cannot run a
    // model.
    for (const gone of ['Context', 'Forward', 'Generator', 'Sampler', 'Adapter']) {
      expect(inferlet).not.toHaveProperty(gone);
    }
  });
});
...
describe('model', () => {
  ...
  it('re-exports the tokenizer surface', () => {
    ...
    // reads `model.encode` the way the Rust `inferlet` crate's does.
...
describe('session', () => {
  ...
  it('returns undefined once the client has closed', () => {
    // Unlike the Python library, which raises, this surface reports end-of-stream
    // as a value. Pinned because the two libraries differ here on purpose.
...
describe('reasoning decoder', () => {
  ...
  it('maps start, delta and complete', () => {
    ...
    // The WIT case is `complete`; the inferlet library calls it `End`. The rename is the
