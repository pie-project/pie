// Public API for the Pie inferlet JavaScript library.
//
...
// same bytes the Rust `inferlet` crate's `eta-dsl` emits for the same program, so a JS
...
// blocks in the call. At run time the JavaScript language component
// (`javascript/inferlet/language`) evaluates the inferlet's source against
// that world.
