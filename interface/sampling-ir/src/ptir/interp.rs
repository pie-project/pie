...
//!
//! Values reuse [`crate::eval::Value`]; unlike the PSIR sampler evaluator,
...
impl Instance {
    ...
    pub fn host_put(&mut self, bound: &BoundTrace, chan: u32, v: Value) -> Result<(), HostError> {
        ...
        let st = &mut self.channels[chan as usize];
    ...
    pub fn len(&self, chan: u32) -> usize {
        self.channels.get(chan as usize).map_or(0, |c| c.queue.len())
    }
    ...
    pub fn step(
        ...
        for e in &bound.readiness {
            let st = &self.channels[e.chan as usize];
            let ok = match e.dir {
        ...
        if committed {
            for (ci, st) in self.channels.iter_mut().enumerate() {
                if ov.taken[ci] {
                ...
                if ov.put[ci] {
                    let v = ov.pending.remove(&(ci as u32)).expect("pending put value");
                    if st.queue.len() >= st.capacity {
                        // A non-leading put into a still-full ring: a
                        // program the fire rule cannot serve — device fault.
                        self.poisoned = true;
...
impl Overlay {
    ...
    fn resolve(&self, inst: &Instance, chan: u32) -> Value {
        ...
        let st = &inst.channels[chan as usize];
...
mod tests {
    use super::*;
    use crate::ptir::container::{ChanDType, ChannelDecl, StageProgram, TraceContainer};
    ...
    fn per_layer_tap_accumulates_via_register_rule() {
        ...
        assert_eq!(inst.channels[0].queue.front().unwrap(), &Value::F32(vec![0.0, 1.0]));
    }
    ...
    fn kernel_fault_poisons() {
        ...
        profile.kernels.push(crate::ptir::registry::KernelInfo {
            name: "boom".into(),
            sink_scope: None,
            replayable: true,
        });
