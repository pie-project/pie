...
use super::container::{ChannelDecl, HostRole, PortSource, TraceContainer};
...
pub fn classify_channels(c: &TraceContainer, readiness: &[ReadinessEntry]) -> Vec<ChannelClass> {
    // Fallible phases: first-use of a host-coupled channel.
    let mut fallible = [false; 5];
    for e in readiness {
        if c.channels[e.chan as usize].host_role != HostRole::None {
...
mod tests {
    use super::*;
    use crate::ptir::container::{ChanDType, ChannelDecl, PortBinding, StageProgram};
    ...
    fn section3() -> TraceContainer {
        ...
        let g = crate::ptir::expand::gumbel(&mut ops, 2, Shape::vector(vocab)); // 4
        let masked = crate::ptir::expand::mask_apply(&mut ops, 1, 3); // 5,6
        let sum = crate::ptir::expand::next_id(&ops);
    ...
    fn spsc_second_producer_rejected() {
        ...
        let id = crate::ptir::expand::next_id(&c.stages[0].ops) - 1;
