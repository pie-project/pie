...
WASM_CRATES=(pie-web runtime engine-wgpu kernels-wgpu web-std wasmtime-web
...
NATIVE_CRATES=(pie-web runtime engine-wgpu kernels-wgpu web-std wasmtime-web
  checkpoint ztensor ztensor-compat tokenizer grammar eta-compiler waker models)
FMT_CRATES=(pie-web runtime engine-wgpu kernels-wgpu web-std wasmtime-web
  checkpoint ztensor ztensor-compat tokenizer grammar eta-compiler waker models)
...
echo "== javascript"
for f in sdk/server/web/src/*.mjs tests/web/*.mjs tests/web/tools/*.mjs benches/web/*.mjs; do
  node --check "$f"
done
for f in tests/web/*.html crates/kernels-wgpu/tools/*.html; do
  node -e '
    const fs = require("fs");
