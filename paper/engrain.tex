...
matcher over 7{,}198 verified rows. We also report where residency is paid for,
including where it loses: at batch 1 we are 0.4--0.7$\times$, on one of three
schemas llguidance is 4.8$\times$ cheaper than us at batch 512, our compiler is
112$\times$ slower than llguidance's at the median, and our lowering accepts
documents its schema rejects 19.6\% of the time --- an audit that located and
fixed a silent \texttt{required}-dropping defect along the way.
\end{abstract}
]
...
ragged batch (\S\ref{sec:sampling}). (v) An evaluation against three engines
that reports the costs of residency --- compile time, memory, small-batch
latency, and a schema on which a baseline beats us 4.8$\times$ --- as
first-class results, and whose correctness audit located and fixed a soundness
defect in our own compiler (\S\ref{sec:correctness}).
...
strictly its better setting and all XGrammar numbers in this paper use it.
...
do not), \textbf{conflicted} (687 rows on grammars with reduce/reduce
...
grammars to sequences and replayed on a different one}). All 7{,}198 rows match,
...
The cause was the order-free object construction. To let properties arrive in
any order while enforcing \texttt{required}, the compiler enumerates subsets of
the required set, one rule per subset. At the \emph{full} subset the rule
picking the next property has no alternatives when the names are also closed
(\texttt{additionalProperties: false}), and the code treated ``no property may
follow'' as a construction failure, falling back to an object that does not
enforce \texttt{required} at all. It is not a failure; it means the object must
close. Minimally, one declared property, required, with
\texttt{additional\-Properties: false} accepted \texttt{\{\}}; with two
properties it did not, which is why no unit test caught it.
...
\begin{table}[t]
\caption{Resident table memory over 528 JSONSchemaBench schemas.
...
\textbf{Compile time} (\S\ref{sec:eval-coverage}) is \emph{linear} in the
number of lexer states (slope 1.05, $r=0.94$): there is no algorithmic blow-up,
only the inherent $O(|V| \times \text{lexer states})$ cost of computing what
every token does from every lexer state. In serving, schemas arrive with
requests, so this is a stated failure and not a footnote.
...
\textbf{Exactness of what.} We report bit-exactness against \emph{our compiled
grammar}, which is weaker and more precise than bit-exactness against the
schema; \S\ref{sec:correctness} measures the difference. We believe every
...
and its cost is flat in the grammar where ours is not. \textbf{Compile is
112$\times$ slower than llguidance} at the median and 35$\times$ slower than
XGrammar, with an unattributed half-hour outlier; lazy lexer residency is the
plausible fix and is unimplemented. \textbf{Tables are 27--700$\times$ larger}
than a host matcher's cache. \textbf{We over-accept 19.6\% of complete
documents} against XGrammar's 5.4\%, and 141 of the 153 remaining cases are
\texttt{required} dropped past the subset budget. \textbf{Fused sampling is
...
size and chunk count was fitted on an A100. \textbf{No end-to-end throughput
claim}: grammar work is a few percent of a decode step and a vLLM A/B at batch
256 ranged from 4{,}523 to 13{,}325 tokens per second across runs, which is too
noisy to attribute.
...
host matcher's p99 rises 7.8$\times$, and is bit-exact over 7{,}198 verified
rows. It also compiles 112$\times$ slower than llguidance, holds 27$\times$ more
memory, loses below batch 32, and loses 4.8$\times$ on one schema of three. We
believe the structural claim is worth those prices, and we have tried to report
them precisely enough that a reader can disagree.
