...
`src/service.rs` declares the `#[tarpc::service] ControlApi` — the unified
control channel for low-rate metadata only (register / report / route / pair).
tarpc over TCP with a bincode codec. `run_as_process` (`src/serve.rs`) serves it
async over an `InProcController` plus a liveness ticker; both workers and the
gateway dial the generated `ControlApiClient`. Workers reach it through the sync
`RemoteController` shim (`src/rpc.rs`), which owns a small runtime so the
worker's boot path stays synchronous; the gateway uses the async client
directly.
...
shared vocabulary (`pie_schema::cluster`), plus tarpc/tokio for the RPC shell —
never on the runtime, transport, or driver crates. The control-RPC surface
(`ControlApi`) is local in `src/service.rs`.
...
over a real tarpc/TCP socket). Wired into the worker: `pie serve --single-node`
embeds the controller in-proc; `--role/--controller` dials a standalone process
over tarpc. Deeper role→stage gating and least-loaded/PD routing are deferred
follow-ons.
