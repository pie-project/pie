...
///
/// Base name comes from `$PIE_SHMEM_NAME` so a launcher (e.g. macOS test
/// harness running two `pie serve` instances in parallel) can namespace
/// each run; default `/pie_shmem` preserves legacy behavior.
